...
# remove keeps data but disables cron reactivation until next install:
# без этого кроны отключённого проекта продолжали бы трогать его логи.
...
if [ "${KEENZOO_UPDATE_LOCK_HELD:-0}" != "1" ]; then
    if [ -d "$LOG_LOCK_DIR" ]; then
        ...
        # dns4.2.19: замок «из будущего» (отрицательный возраст) тоже
        # считаем битым и снимаем, иначе ротация встала бы навсегда.
        [ $((_rl_now - _rl_t)) -gt 300 ] || [ $((_rl_now - _rl_t)) -lt 0 ] || exit 0
        rm -rf "$LOG_LOCK_DIR"
    fi
