...
# dns4.2.36 (аудит, KZ-07): непредсказуемое имя временного статусного
# файла; переименование атомарно и не следует по чужому симлинку.
TMP_STATUS="$(mktemp "${STATUS_FILE}.XXXXXX" 2>/dev/null || printf '%s' "${STATUS_FILE}.$$")"
LOCK_ACQUIRED=0
ROLLBACK_DIR=""
COMMITTED_SETS=""
STATIC_SETS=""
...
rollback_update() {
    ...
    if [ "$_fu_ok" -eq 1 ]; then
        discard_staging
        write_status "$_fu_status" "$_fu_msg"
    else
        KEEP_ROLLBACK=1
        _fu_msg="$_fu_msg; откат частичный или не подтверждён: снапшот сохранён в ${ROLLBACK_DIR}; нужна ручная проверка или повторный деплой"
        write_status "rollback_failed" "$_fu_msg"
        _fu_status="rollback_failed"
    fi
    # Единая строка терминального статуса: причина раньше оставалась только в
    # JSON-статусе; в логах крон-запусков (/tmp/unblock_update.log) и syslog
    # её не было. Статус «done» не журналируется — он не несёт инцидентной
    # семантики и шумел бы каждый прогон.
    if [ "$_fu_status" != "done" ]; then
        logger -t "unblock_update" "finish ${_fu_status}: ${_fu_msg}" || true
        echo "finish ${_fu_status}: ${_fu_msg}" >&2
    fi
    return 0
}
...
defer_update() {
    # EX_TEMPFAIL (75): временная недоступность DNS-среды, а не сбой
    # транзакции. Откат возвращает прежнее рабочее состояние, статус —
    # «defer», повтор — следующее окно по расписанию; текстовый вид плашки
    # в панели — «⏳ Отложено: <причина>».
    rollback_update "$1" "defer"
    exit 75
}
...
lock_owner_live() {
    if [ ! -f "$SCRIPT_LOCK/pid" ]; then
        ...
        # dns4.2.19: отрицательный возраст (скачок часов назад) — не
        # «живой» замок: иначе прогон ждал бы до 300 с и умирал "locked".
        [ $((_lol_now - _lol_mtime)) -ge 0 ] \
...
# dns4.2.38 N-1: списки обязаны заканчиваться переводом строки — иначе
# следующее дописывание через «>>» склеит последнюю строку с первой
# новой (так в списке бота появился несуществующий «telegram.me2ip.ru»,
# а настоящий домен выпал из обхода). Только диагностика: логика
# обработки не меняется, битый файл продолжает обрабатываться как есть.
for _lst in /opt/etc/unblock/*.txt; do
    [ -s "$_lst" ] || continue
    [ -z "$(tail -c1 "$_lst")" ] \
        || logger -t "unblock_update" "warning: $_lst has no trailing newline"
done
...
# rc=75 от стадии кандидата (см. KEENZOO_DNS_STAGE в unblock_dnsmasq.sh) —
# DNS-среда недоступна («fail closed» временно), это defer, а не ошибка.
_sg_rc=0
DNS_HEALTH_LOG="$DNS_STAGE_LOG" KEENZOO_DNS_STAGE=1 \
    KEENZOO_STAGE_CIDR=1 IPSET_SUFFIX="" /opt/bin/unblock_dnsmasq.sh || _sg_rc=$?
case "$_sg_rc" in
    0) ;;
    75) defer_update "генерация кандидата dnsmasq отложена: DNS-среда недоступна; повтор по расписанию" ;;
...
# bot.txt/vless.txt.
# rc=75 (EX_TEMPFAIL) от unblock_ipset.sh — временная недоступность DNS
# (снапшот/деградация резолва): откладываем всю транзакцию, а не фиксируем
# ошибку. Коды 0/75/1 разводятся явно; «|| _sf_rc=$?» обязателен под set -eu.
_sf_rc=0
DNS_HEALTH_LOG="$DNS_STAGE_LOG" KEENZOO_USE_DNS_SNAPSHOT=1 SKIP_VPN=1 IPSET_SUFFIX="_new" \
    /opt/bin/unblock_ipset.sh || _sf_rc=$?
case "$_sf_rc" in
    0) ;;
    75) defer_update "заполнение списков отложено: DNS временно недоступен или снапшот не подтверждён; повтор по расписанию" ;;
...
ipset_member_count() {
    _imc_set="$1"
    # dns4.2.37 П-6: `ipset list <set>` выгружал ВСЕХ участников набора
    # (десятки тысяч строк на больших списках, O(N)) ради одной цифры.
    # Заголовок `ipset list -t <set>` содержит готовую строку
    # «Number of entries:» — O(1), без временного файла и awk-прохода.
    if ! _imc_term="$(ipset list -t "$_imc_set" 2>/dev/null \
        | sed -n 's/^Number of entries:[[:space:]]*//p' | head -1)"; then
        return 1
    fi
    case "$_imc_term" in
        '' | *[!0-9]*) return 1 ;;
...
commit_set() {
    ...
    if ! awk -v set="$_cs_set" '
            ...
            inside && NF {
                entry = $1
                # dns4.2.27 (внешний аудит, U2-02): merge — второй путь
                # попадания записей в живой набор, поэтому каждая
                # перевалидировается. Принимаются только корректные
                # IPv4-CIDR; 0.0.0.0/0 и зарезервированные диапазоны
                # отбрасываются (те же диапазоны, что в каноническом
                # is_public_cidr из unblock_dnsmasq.sh). Иначе одна
                # плохая запись в старом наборе переживала бы каждый
                # merge и раскрывала бы список обхода.
                if (entry ~ /^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+\/[0-9]+$/) {
                    slash = index(entry, "/")
                    ip = substr(entry, 1, slash - 1)
                    p = substr(entry, slash + 1) + 0
                } else if (entry ~ /^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$/) {
                    # ipset печатает одиночный узел (/32) без префикса
                    ip = entry
                    p = 32
                } else next
                if (p < 0 || p > 32) next
                n = split(ip, _ao, ".")
                if (n != 4) next
                if (_ao[1] > 255 || _ao[2] > 255 || _ao[3] > 255 || _ao[4] > 255) next
                v = ip2int(ip)
                block = 1
                ...
                if (overlap(first,last,0,16777215) || \
                    ...
                    overlap(first,last,3758096384,4294967295)) next
                print "add " set " " entry " -exist"
            }
...
case "$_upd_final" in
    # EMERGENCY_DNS — аварийный public DNS без DNSSEC, но среда жива:
    # клиенты и ipset-наполнение идут по тем же транспортам (см. v4_shell
    # DNS_INSECURE_PORTS), поэтому списки публикуются, как и в давно
    # допущенном не-DNSSEC режиме DNS_OK_NO_DNSSEC. Иначе каждое
    # обновление падало «DNS candidate missing or invalid», и списки
    # замораживались на весь период аварии DNSSEC.
    *' mode=TUNNEL_DNS '*|*' mode=LOCAL_DNSSEC '*|*' mode=DNS_OK_NO_DNSSEC '*|*' mode=EMERGENCY_DNS '*) ;;
    *' mode=DNS_UNAVAILABLE '*)
        # Решение честно записано, но среда недоступна — defer.
        defer_update "кандидат DNS зафиксирован как DNS_UNAVAILABLE: среда недоступна; повтор по расписанию" ;;
    *)
