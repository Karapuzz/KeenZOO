...
# dns4.2.36 (аудит, KZ-11): rollback-копии бинарников (~21 МБ) и
# скачанные архивы не должны лежать в tmpfs (/tmp = RAM роутера).
# Используем постоянный /opt, если он доступен; /tmp — только запасной.
upd_stage_dir() {
    if [ -d /opt/tmp ] || mkdir -p /opt/tmp 2>/dev/null; then
        printf '/opt/tmp'
    else
        printf '/tmp'
    fi
...
restore_xray_state() {
    ...
    wait_service_ready xray "$PORT_VLESS" udp || true
}
...
restore_hysteria_state() {
    ...
    wait_service_ready hysteria "$PORT_HYSTERIA" tcp || true
}
...
update_xray() {
    choose_xray_source
    XRAY_OLD_BIN="$(mktemp "$(upd_stage_dir)/xray.rollback.XXXXXX")"
    XRAY_OLD_CFG="$(mktemp "$(upd_stage_dir)/xray.config.rollback.XXXXXX")"
    XRAY_OLD_BIN_OK=0
    XRAY_OLD_CFG_OK=0
    ...
    if [ "$XRAY_SOURCE" = "opkg" ]; then
        ...
        # dns4.2.36 (аудит, KZ-02): проверяем ОБА входа — и TCP-редирект,
        # и UDP-ТПРОКСИ на том же порту.
        if ! start_xray || ! wait_service_ready xray "$PORT_VLESS" tcp \
                || ! wait_service_ready xray "$PORT_VLESS" udp; then
    ...
    if [ "$XRAY_SOURCE" = "github" ] && [ -n "$XRAY_FILE" ]; then
        TMP_ZIP="$(mktemp "$(upd_stage_dir)/xray.zip.XXXXXX")"
        TMP_DGST="$(mktemp "$(upd_stage_dir)/xray.dgst.XXXXXX")"
        TMP_DIR="$(mktemp -d "$(upd_stage_dir)/xray.XXXXXX")"
...
update_hysteria() {
    HY_OLD_BIN="$(mktemp "$(upd_stage_dir)/hysteria.rollback.XXXXXX")"
    HY_OLD_CFG="$(mktemp "$(upd_stage_dir)/hysteria.config.rollback.XXXXXX")"
    HY_OLD_BIN_OK=0
    HY_OLD_CFG_OK=0
    ...
    TMP_BIN="$(mktemp "$(upd_stage_dir)/hysteria.bin.XXXXXX")"
    TMP_HASHES="$(mktemp "$(upd_stage_dir)/hysteria.hashes.XXXXXX")"
    ...
    if [ "$_ready" = "1" ]; then
        ...
        # dns4.2.36 (аудит, KZ-02): tcpRedirect и udpTProxy обязаны быть оба.
        if ! start_hysteria || ! wait_service_ready hysteria "$PORT_HYSTERIA" udp \
                || ! wait_service_ready hysteria "$PORT_HYSTERIA" tcp; then
