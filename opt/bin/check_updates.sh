...
cu_is_mips_le() {
    ...
    # зонд 6-го байта ELF через `od` ИЛИ `hexdump` + расширенный шаблон
    # /proc/cpuinfo («byteorder : little»). Дефолт прежний: считать BE
    # (консервативный путь обновлений).
    for _cu_c in /bin/busybox /bin/sh /bin/cat; do
        ...
        if command -v od >/dev/null 2>&1; then
            ...
            _cu_b="$(dd if="$_cu_c" bs=1 skip=5 count=1 2>/dev/null | hexdump -e '1/1 "%d"')"
            ...
            _cu_b="$(dd if="$_cu_c" bs=1 skip=5 count=1 2>/dev/null | busybox hexdump -e '1/1 "%d"')"
        fi
        case "$_cu_b" in
            1) return 0 ;;
            2) return 1 ;;
