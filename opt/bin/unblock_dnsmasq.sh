...
update_dynamic_interfaces() {
    ...
    _udi_local=""
    # dns4.2.21 (A3 H1): routerip валидируется как IPv4 до матча, а матч
    # идёт дословно (grep -F) — метасимволы . * [ ] из конфига не должны
    # ложно матчить чужой адрес и не должны ломать детект интерфейса.
    if [ -n "$_udi_router" ] && ! is_ip "$_udi_router"; then
        log_msg "ERROR: invalid routerip '$_udi_router' (not IPv4), ignored" \
            "update_dynamic_interfaces"
        _udi_router=""
    fi
