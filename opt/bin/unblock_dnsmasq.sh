...
run_bounded() {
    _rb_seconds="$1"
    shift
    # dns4.2.32 (статический аудит, F-09): используется УЖЕ найденный
    # интерпретатор — литеральный «python3» пропускал версионированные
    # сборки без симлинка, и deadline-раннер молча отключался.
    if command -v "$KZ_PY" >/dev/null 2>&1; then
    "$KZ_PY" - "$_rb_seconds" "$@" <<'PY_DEADLINE'
import os
import signal
import subprocess
import sys
...
# is_public_ipv4/is_public_cidr/is_public_range в ДРУГОМ файле (пары:
# unblock_ipset.sh / unblock_dnsmasq.sh) и _RESERVED_NETS в /opt/etc/bot/generator.py.
is_public_cidr() {
    _ipc_entry="$1"
    case "$_ipc_entry" in
...
# dns4.2.19: управляемый блок фильтра AAAA-ответов. Владельцем файла
# является этот скрипт, поэтому маркерами владеет он же (purge в
# deploy_bypass.sh снимает их тем же префиксным шаблоном).
DNS_FAAAA_BEGIN="# BEGIN KeenZOO filter-aaaa"
DNS_FAAAA_END="# END KeenZOO filter-aaaa"
...
update_dynamic_interfaces() {
    ...
    # dns4.2.27 (внешний аудит, Q-04): запасной путь для экзотических
    # сборок, где «-o -4» вместе отдаёт пусто: повтор без «-4» с отбором
    # только inet-строк. На штатных прошивках работает первый вариант.
    _udi_dump="$(ip -o -4 addr show 2>/dev/null || true)"
    if [ -z "$_udi_dump" ]; then
        _udi_dump="$(ip -o addr show 2>/dev/null \
            | awk '$3 == "inet"' || true)"
    fi
    _udi_links="$(printf '%s\n' "$_udi_dump" | awk '{print $2}' \
        | sed 's/@.*//' | tr '\n' ' ')"
    _udi_add_if() {
        [ -n "$1" ] || return 0
    ...
    _udi_local=""
    if [ -n "$_udi_router" ]; then
        # dns4.2.22: self-contained проверка IPv4. Прежний вариант
        # dns4.2.21 использовал is_ip — но она объявляется НИЖЕ точки
        # вызова update_dynamic_interfaces, поэтому в момент исполнения
        # не существовала (rc=127, инверсия TRUE на валидном адресе), а
        # log_msg вообще принадлежит 100-redirect.sh — под set -eu весь
        # скрипт тихо умирал 127-м на типовом конфиге (аудит N1).
        # Здесь нет внешних зависимостей вообще: awk есть везде.
        if ! printf '%s\n' "$_udi_router" | awk -F. '
            { if (NF != 4) exit 1
              for (i = 1; i <= 4; i++)
        ...
        then
            logger -t "unblock_dnsmasq" \
                "invalid routerip '$_udi_router' (not IPv4), ignored"
            _udi_router=""
        fi
    fi
    # dns4.2.21 (A3 H1): матч — дословный (grep -F): метасимволы . * [ ]
    # конфига не должны ложно матчить чужой адрес и ломать детект.
    if [ -n "$_udi_router" ] && ip -4 addr show 2>/dev/null \
        | grep -qF " $_udi_router/"; then
...
# dns4.2.19: статическая санация базового dnsmasq.conf. На IPv4-only
# парке устаревшие IPv6-строки от прежних установок (зональные upstreams,
# listen/except с адресом-«:», AААA-формы) не должны попадать в конфиг:
# dnsmasq без IPv6 стартует с шумом, а AAAA-ответы дают клиентам путь
# в обход туннеля. Работает ТОЛЬКО вне управляемых блоков и только по
# однозначно-IPv6 формам: шаблоны требуют «:» в значении, поэтому IPv4,
# server=IPv4, server=/zone/IPv4#port и listen-address=IPv4 невозможно
# задеть. Комментарии сохраняются (в т.ч. исторические #server=...).
sanitize_dnsmasq_conf() {
    [ -f "$DNSMASQ_CONF" ] || return 0
    _sdc_tmp="${DNSMASQ_CONF}.sanitize.$$"
    awk '
        /^#[[:space:]]*BEGIN KeenZOO/ { inblk = 1; print; next }
        /^#[[:space:]]*END KeenZOO/   { inblk = 0; print; next }
        inblk  { print; next }
        /^[[:space:]]*#/ { print; next }
        /^[[:space:]]*server=[^#]*:/        { next }
        /^[[:space:]]*address=\/[^ ]*:[^ ]*/{ next }
        /^[[:space:]]*listen-address=.*:/   { next }
        /^[[:space:]]*except-interface=.*:/ { next }
        /^[[:space:]]*filter-aaaa([[:space:]]|$)/ { next }
        { print }
    ' "$DNSMASQ_CONF" > "$_sdc_tmp" \
        && mv -f "$_sdc_tmp" "$DNSMASQ_CONF" || rm -f "$_sdc_tmp"
}

# dns4.2.19: управляемый блок «filter-aaaa». Директива существует в
# dnsmasq >= 2.86; поддержку определяем синтаксическим тестом временного
# конфига (надёжнее --version на backport-сборках). На старых сборках
# блок не пишем — и срезаем, если он уже есть, иначе dnsmasq не стартует.
update_filter_aaaa() {
    [ -f "$DNSMASQ_CONF" ] || return 0
    _ufa_ok=0
    if command -v dnsmasq >/dev/null 2>&1; then
        # dns4.2.21 (A3 C2): предсказуемое имя в world-writable /tmp =
        # symlink-перезапись чужого файла; только mktemp. Если mktemp
        # недоступен — пробу не делаем вообще (fail-closed по _ufa_ok).
        _ufa_test="$(mktemp /tmp/keenzoo_faaaa_test.XXXXXX 2>/dev/null || true)"
        if [ -n "$_ufa_test" ]; then
            printf 'filter-aaaa\n' > "$_ufa_test" 2>/dev/null \
                && dnsmasq --test -C "$_ufa_test" >/dev/null 2>&1 \
                && _ufa_ok=1
            rm -f "$_ufa_test"
        fi
    fi
    _ufa_tmp="${DNSMASQ_CONF}.faaaa.$$"
    awk -v b="$DNS_FAAAA_BEGIN" -v e="$DNS_FAAAA_END" -v ok="$_ufa_ok" '
        BEGIN { inblk = 0; seen = 0 }
        $0 == b {
            inblk = 1; seen = 1
            if (ok == 1) {
                print
                print "# dns4.2.19: IPv4-only парк. AAAA-ответы клиентам"
                print "# фильтруются — нет IPv6-пути в обход туннеля."
                print "filter-aaaa"
            }
            next
        }
        $0 == e { inblk = 0; if (ok == 1) print; next }
        inblk { next }
        { print }
        END {
            if (!seen && ok == 1) {
    ...
    ' "$DNSMASQ_CONF" > "$_ufa_tmp" \
        && mv -f "$_ufa_tmp" "$DNSMASQ_CONF" || rm -f "$_ufa_tmp"
}

# Порядок важен: санация — до блок-редактирования (чтобы update_* видели
# чистый базовый файл), filter-aaaa — после: его строка живёт вне блоков
# update_dynamic_interfaces и не должна быть удалена санацией при следующем
# прогоне (она — внутри собственных маркеров).
sanitize_dnsmasq_conf
update_dynamic_interfaces
update_filter_aaaa
    ...
    printf '%s\n' "$_ipi" | awk -F. '
        {
            ...
            if (a == 192 && b == 88 && $3 == 99) bad=1
            if (a == 192 && b == 175 && $3 == 48) bad=1
...
if [ -z "$IPTABLES_DNS_BIN" ]; then
    for _idc in /usr/sbin/iptables /sbin/iptables /bin/iptables \
        ...
        # dns4.2.21 (A3 C1): /tmp world-writable кандидаты исключены
        # (hijack iptables); symlink того же бинаря отклоняется.
        [ -x "$_idc" ] && [ ! -L "$_idc" ] || continue
...
DNS_POLICY_V4=1
v4_load_dns() {
    _v4_state="$("$KZ_PY" /opt/etc/bot/utils.py --dns-shell)" || return 1
    # dns4.2.27 (внешний аудит, Q-02): разбор БЕЗ eval — построчно с
    # белым списком имён (полный набор ключей выдаёт utils.v4_shell).
    # Значения там ограничены перечислениями и константами портов;
    # кавычки снимаются при присваивании. Неизвестные строки
    # отбрасываются, а не исполняются.
...
# dns4.2.37 M-8: лёгкое ожидание контроллера БЕЗ запуска интерпретатора.
# Раньше каждая секунда ожидания делала полный `utils.py --dns-shell`
# (до 60 стартов питона на MIPS). Теперь готовность определяется прямо
# по статусному JSON: свежий `epoch` и валидный `mode`. Это только
# предварительная проверка выхода из цикла — итоговое, авторитетное
# чтение с проверкой живости pid/идентичности делает один вызов
# v4_load_dns в конце.
_v4_poll_ready() {
    _vp_file=/tmp/keenzoo-dns-v4.json
    [ -r "$_vp_file" ] || return 1
    _vp_epoch="$(grep -o '"epoch"[[:space:]]*:[[:space:]]*[0-9]*' \
        "$_vp_file" 2>/dev/null | head -1 | grep -o '[0-9]*$')"
    [ -n "$_vp_epoch" ] || return 1
    _vp_age=$(( $(date +%s) - _vp_epoch ))
    # Тот же допуск свежести, что у v4_status: не младше -2 с и не старше
    # 180 с. Старый файл от умершего контроллера «готовым» не считается.
    [ "$_vp_age" -ge -2 ] && [ "$_vp_age" -le 180 ] || return 1
    _vp_mode="$(grep -o '"mode"[[:space:]]*:[[:space:]]*"[^"]*"' \
        "$_vp_file" 2>/dev/null | head -1)"
    case "$_vp_mode" in
        *LOCAL_DNSSEC* | *TUNNEL_DNS* | *EMERGENCY_DNS*) return 0 ;;
    esac
    return 1
}
...
v4_start_dns() {
    ...
    "$KZ_PY" /opt/etc/bot/utils.py --dns-start || return 1
    _v4_wait=0
    while [ "$_v4_wait" -lt 60 ]; do
        _v4_poll_ready && break
        sleep 1
        _v4_wait=$((_v4_wait + 1))
    done
    # Один авторитетный разбор через интерпретатор: живость контроллера,
    # идентичность и полный набор переменных. Если контроллер так и не
    # готов — DNS_MODE останется DNS_UNAVAILABLE, поведение прежнее.
...
ndm_dot_tls_probe() {
    ...
    command -v "$KZ_PY" >/dev/null 2>&1 || return 127
...
ndm_endpoint_reachable() {
    ...
    if [ "$_ner_port" = "853" ]; then
        if ndm_dot_tls_probe "$_ner_host" "$_ner_ip"; then
            logger -t "unblock_dnsmasq" \
                "NDM probe: host=$_ner_host ip=$_ner_ip level=TLS_OK"
            return 0
        else
            _ner_probe_rc=$?
        fi
        case "$_ner_probe_rc" in
            125|126|127)
                logger -t "unblock_dnsmasq" \
                    "NDM probe: host=$_ner_host ip=$_ner_ip level=PROBE_UNAVAILABLE rc=$_ner_probe_rc"
                return 1 ;;
        esac
        logger -t "unblock_dnsmasq" \
            "NDM probe: host=$_ner_host ip=$_ner_ip level=TLS_UNAVAILABLE"
        return 1
    fi
    ...
    if [ "$_ner_rc" -eq 0 ]; then
        case "$_ner_code" in
            ...
            400|405)
                logger -t "unblock_dnsmasq" \
                    "NDM probe: host=$_ner_host ip=$_ner_ip level=HTTP_REACHED code=$_ner_code"
                rm -f "$_ner_trace"
                return 0
                ;;
            401|403|407)
                logger -t "unblock_dnsmasq" \
                    "NDM probe: host=$_ner_host ip=$_ner_ip level=HTTP_DENIED code=$_ner_code"
                rm -f "$_ner_trace"
                return 1
                ;;
            4*)
                logger -t "unblock_dnsmasq" \
                    "NDM probe: host=$_ner_host ip=$_ner_ip level=HTTP_REJECTED code=$_ner_code"
                rm -f "$_ner_trace"
                return 1
                ;;
            5*)
                logger -t "unblock_dnsmasq" \
                    "NDM probe: host=$_ner_host ip=$_ner_ip level=HTTP_UPSTREAM code=$_ner_code"
                rm -f "$_ner_trace"
                return 1
                ;;
        esac
    ...
    logger -t "unblock_dnsmasq" \
        "NDM probe: host=$_ner_host ip=$_ner_ip level=$_ner_level rc=$_ner_rc"
    rm -f "$_ner_trace"
    return 1
}
...
"
    ...
    if [ "$_pin_sig" != "$_pin_prev" ]; then
        ...
        # dns4.2.36 (аудит, KZ-07): запись через непредсказуемый
        # временный файл и переименование (не следует по симлинку).
        _pin_tmp="$(mktemp /tmp/keenzoo_pin.sig.XXXXXX 2>/dev/null || true)"
        if [ -n "$_pin_tmp" ]; then
            if printf '%s\n' "$_pin_sig" > "$_pin_tmp" 2>/dev/null; then
                mv -f "$_pin_tmp" /tmp/keenzoo_pin.sig 2>/dev/null \
...
process_dnsmasq_list() {
    ...
    # dns4.2.39 N-7 (аудит-2, П-1): ОДИН проход awk на файл списка вместо
    # ~8 форков на строку (внешние sed/tr/awk/grep в цепочке
    # trim_comment → is_ip → is_cidr/is_public_cidr → is_public_range →
    # normalize_domain_mode → dnsmasq_resource_server). Сигнатура,
    # глобалы (TMP_OUT/TMP_CIDR/DNS_BACKUP_PORTS/DNS_MODE/DNSMASQ_CONF),
    # выходные строки и сообщения журнала — прежние; валидация — те же
    # правила, что в is_ip/is_cidr/is_public_*/is_domain_core (только
    # BusyBox-awk: без gensub, интервалов {n,m} и побитовых сдвигов).
    # Отличие от исходного патча аудита: зоны из dnsmasq.conf читаются
    # через getline в BEGIN, а не схемой «два файла, первый — конфиг»:
    # при пустом/отсутствующем конфиге счётчик файлов не переключился бы
    # и весь список разбирался бы как конфиг (правила терялись).
    _pdl_log="$(mktemp /tmp/unblock.dnsmasq.log.XXXXXX 2>/dev/null)" || return 1
    _pdl_conf="$DNSMASQ_CONF"
    [ -f "$_pdl_conf" ] || _pdl_conf=/dev/null

    awk -v setname="$_setname" -v port="$_dns_port" \
        -v backups="$DNS_BACKUP_PORTS" -v mode="$DNS_MODE" \
        -v out="$TMP_OUT" -v cidr_out="$TMP_CIDR" -v log_out="$_pdl_log" \
        -v conf="$_pdl_conf" '
    function is_ip(v,   a, n, i) {
        n = split(v, a, ".")
        if (n != 4) return 0
        for (i = 1; i <= 4; i++) {
            if (a[i] !~ /^[0-9]+$/) return 0
            if (length(a[i]) > 3) return 0
    ...
    function overlap(a, b, c, d) { return a <= d && b >= c }
    function non_public(first, last) {
        return (overlap(first,last,0,16777215) ||
            overlap(first,last,167772160,184549375) ||
            overlap(first,last,1681915904,1686110207) ||
            overlap(first,last,2130706432,2147483647) ||
            overlap(first,last,2851995648,2852061183) ||
            ...
            overlap(first,last,3325256704,3325256959) ||
            overlap(first,last,3405803776,3405804031) ||
            overlap(first,last,3758096384,4294967295))
    }
    function is_public_cidr(ip, p,   v, block, i, first, last) {
        if (p < 0 || p > 32) return 0
        v = ip2int(ip); block = 1
        for (i = 0; i < 32 - p; i++) block *= 2
        first = int(v / block) * block; last = first + block - 1
        return !non_public(first, last)
    }
    function is_public_range(a, b,   first, last) {
        first = ip2int(a); last = ip2int(b)
        if (first > last) return 0
        return !non_public(first, last)
    }
    function is_domain_core(s,   a, n, i, lbl, L, j, ch) {
        s = tolower(s)
        if (s == "" || length(s) > 253) return 0
        if (index(s, "/") > 0 || index(s, " ") > 0) return 0
        n = split(s, a, ".")
        if (n < 2) return 0
        for (i = 1; i <= n; i++) {
            lbl = a[i]; L = length(lbl)
            if (L == 0 || L > 63) return 0
            if (substr(lbl, 1, 1) == "-" || substr(lbl, L, 1) == "-") return 0
            for (j = 1; j <= L; j++) {
                ch = substr(lbl, j, 1)
                if (index("abcdefghijklmnopqrstuvwxyz0123456789-", ch) == 0) return 0
            }
        }
        if (a[n] ~ /^[0-9]+$/) return 0
        return 1
    }
    function covered(h,   z) {
        for (z in zones) {
            if (h == z) return 1
            if (length(h) > length(z) && substr(h, length(h) - length(z)) == "." z) return 1
        }
        return 0
    }
    BEGIN {
        # Пользовательские/специальные зоны server=/a/b/… из
        # dnsmasq.conf (аналог dnsmasq_resource_server). Читаются до
        # списка, поэтому пустой или отсутствующий конфиг просто не
        # даёт зон и не влияет на разбор списка.
        if (conf != "/dev/null") {
            while ((getline cline < conf) > 0) {
                if (cline ~ /^[[:space:]]*server=\/[^#]/) {
                    sub(/^[[:space:]]*server=\//, "", cline)
                    zn = split(cline, zz, "/")
                    for (zi = 1; zi < zn; zi++) if (zz[zi] != "") zones[tolower(zz[zi])] = 1
                }
            }
            close(conf)
        }
    }
    {
        ...
        sub(/[[:space:]]+$/, "", line)
        if (line == "") next
        if (is_ip(line)) next                       # голые IP кладёт unblock_ipset.sh
        if (index(line, "/") > 0) {                 # is_cidr
            ip = line; sub(/\/.*$/, "", ip)
            p = line; sub(/^[^\/]*\//, "", p)
            if (p ~ /^[0-9]+$/ && length(p) <= 2 && p + 0 <= 32 && is_ip(ip)) {
                if (is_public_cidr(ip, p + 0)) print "add " setname " " line >> cidr_out
                else print "skip non-public network: " line >> log_out
                next
            }
        }
        # Диапазоны вида a.b.c.d-e.f.g.h тоже обрабатывает ipset-скрипт.
        if (line ~ /[0-9]-[0-9]/ && line ~ /^[0-9.]+-[0-9.]+$/) {
            ra = line; sub(/-.*$/, "", ra)
            rb = line; sub(/^[^-]*-/, "", rb)
            if (is_ip(ra) && is_ip(rb) && is_public_range(ra, rb)) next
            print "skip non-public range: " line >> log_out
            next
        }
        host = tolower(line); sub(/\.$/, "", host)
        if (substr(host, 1, 2) == "*.") host = substr(host, 3)
        else if (substr(host, 1, 1) == ".") host = substr(host, 2)
        if (!is_domain_core(host)) { print "skip invalid entry: " line >> log_out; next }
        print "ipset=/" host "/" setname >> out
        if (covered(host)) { print "DNS resource rule preserved: " host " from " conf >> log_out; next }
        if (port == "") { print "DNS resource upstream omitted: " host " mode=" mode >> log_out; next }
        if (port == "9053") { print "server=/" host "/127.0.0.1#9053" >> out; next }
        print "server=/" host "/127.0.0.1#" port >> out
        nb = split(backups, bp, " ")
        for (bi = 1; bi <= nb; bi++) if (bp[bi] != "") print "server=/" host "/127.0.0.1#" bp[bi] >> out
    }' "$_file"
    _pdl_rc=$?

    # Сообщения журнала — как раньше, по одному вызову logger на событие
    # (их единицы; в обычном списке ни одного).
    if [ -s "$_pdl_log" ]; then
        while IFS= read -r _pdl_msg; do
            logger -t "unblock_dnsmasq" "$_pdl_msg"
        done < "$_pdl_log"
    fi
    rm -f "$_pdl_log"
    return "$_pdl_rc"
...
if [ -s "$TMP_CIDR" ]; then
    ...
    if [ "${KEENZOO_STAGE_CIDR:-0}" != "1" ]; then
        # dns4.2.38 N-5: прежние два цикла «create -exist + del» и
        # «create -exist + add» стоили по 4 запуска ipset на каждую
        # подсеть (120 на 30 подсетей); «del» перед «add» бесполезен для
        # hash:net без timeout. Один проход awk в файл формата, который
        # уже использует unblock_ipset.sh, и один `ipset restore -exist`;
        # итоговое состояние наборов идентично (проверено дампом).
        _cidr_restore="${TMP_CIDR}.restore"
        awk -v sfx="$IPSET_SUFFIX" '
            $2 != "" && $3 != "" {
                s = $2 sfx
                if (!(s in seen)) { seen[s] = 1; print "create " s " hash:net family inet" }
                print "add " s " " $3
            }' "$CIDR_FILE" > "$_cidr_restore"
        if [ -s "$_cidr_restore" ]; then
            ipset restore -exist < "$_cidr_restore" 2>/dev/null \
                || logger -t "unblock_dnsmasq" "ipset restore (CIDR) failed" \
                2>/dev/null || true
        fi
        rm -f "$_cidr_restore"
    fi
