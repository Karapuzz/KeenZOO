...
restore_dns() {
    ...
    if [ -n "$RESOLV_LINK" ]; then
        # Симлинк: структура важнее содержимого — файл-цель
        # управляется прошивкой и будет перезаписан ею.
...
# dns4.2.33 (статический аудит, F-01): миграция специальных зон на
# контроллер. Старые установки сохраняют прямой апстрим зон (конфиг
# сохраняется при апгрейде), но прямой внешний DNS на :53 отныне закрыт
# фильтром — зоны направляются на петлевую фасаду контроллера (список
# резолверов зон — в utils.py, V4_ZONE_SERVERS). Идемпотентно: строки,
# уже ведущие на контроллер, не трогаются.
if [ -f /opt/etc/dnsmasq.conf ]; then
    _kz_zone_mig=0
    for _kz_zone in \
        lib emc coin bazar geek libre bbs chan cyb indy neo null pirate; do
        if grep -q "^server=/$_kz_zone/" /opt/etc/dnsmasq.conf \
            && ! grep -q "^server=/$_kz_zone/127\.0\.0\.1#40512$" \
                /opt/etc/dnsmasq.conf; then
            sed -i "s|^server=/$_kz_zone/.*|server=/$_kz_zone/127.0.0.1#40512|" \
                /opt/etc/dnsmasq.conf 2>/dev/null || true
            _kz_zone_mig=1
        fi
    done
    if [ "$_kz_zone_mig" = 1 ]; then
        # /lib исторически имел четыре апстрима — после перезаписи
        # одинаковые строки удаляются с сохранением порядка.
        _kz_mig_tmp="$(mktemp /tmp/dnsmasq.mig.XXXXXX)"
        if awk '{
                if ($0 ~ /^server=\/(lib|emc|coin|bazar|geek|libre|bbs|chan|cyb|indy|neo|null|pirate)\/127\.0\.0\.1#40512$/ \
                    && _kz_seen[$0]++) next
                print
        ...
        echo "✅ Специальные зоны направлены через контроллер"
    fi
fi
