...
cleanup_deploy_lock() {
    _cdl_rc=$?
    # dns4.2.32 (статический аудит, F-07): резолвер возвращается и при
    # die/сигнале. Гард по флагу: до вызова ensure_dns переменная не
    # установлена, а функция восстановления ещё не определена.
    if [ "${RESOLV_MODIFIED:-0}" = 1 ]; then
        restore_dns
    fi
...
remove_managed_file_sections() {
    ...
    awk '
        ...
        $0 == "# BEGIN KeenZOO special zones" ||
        $0 == "# BEGIN KeenZOO filter-aaaa" ||
        ...
        $0 == "# END KeenZOO special zones" ||
        $0 == "# END KeenZOO filter-aaaa" ||
...
verify_project_removed() {
    ...
    if command -v iptables-save >/dev/null 2>&1; then
        if iptables-save 2>/dev/null | grep -Eq \
            'unblock(sh|tor|vless|troj|hysteria|router|dns)|TPROXY|priority 1770|KZ_DNS_V4_FILTER'; then
            warn "⚠️ В iptables остались project rules"
            _vpr_ok=0
        fi
    fi
    # dns4.2.35 (ре-аудит, R-11): проверка и проектных IPv6-правил —
    # после корректного удаления порты не должны оставаться закрытыми.
    if command -v ip6tables-save >/dev/null 2>&1; then
        if ip6tables-save 2>/dev/null | grep -Eq \
            -- "--dport ($PORT_SS|$PORT_TOR|$PORT_VLESS|$PORT_TROJAN|$PORT_HYSTERIA|$WEB_PORT) .*-j (DROP|ACCEPT)"; then
            warn "⚠️ В ip6tables остались project rules"
            _vpr_ok=0
        fi
    fi
...
restore_direct_zone_upstreams() {
    # dns4.2.35 (ре-аудит, R-03): специальные зоны публичными
    # резолверами не резолвятся. После удаления проекта контроллер
    # (127.0.0.1#40512) исчезает; чтобы зоны не остались без
    # резолверов, возвращаем прямые резолверы зон (строки без
    # управляемых маркеров — проект удалён, управлять ими некому).
    _rz_conf="$1"
    [ -f "$_rz_conf" ] || return 0
    if grep -Eq '^server=/(lib|emc|coin|bazar|geek|libre|bbs|chan|cyb|indy|neo|null|pirate)/' \
            "$_rz_conf"; then
        return 0
    fi
    {
        printf '%s\n' \
            'server=/lib/65.21.1.106' \
            'server=/lib/95.216.99.249' \
            'server=/lib/94.247.43.254' \
            'server=/lib/195.10.195.195'
        for _rz_z in emc coin bazar geek libre bbs chan cyb indy neo null pirate; do
            printf 'server=/%s/95.216.99.249\n' "$_rz_z"
        done
    } >> "$_rz_conf" 2>/dev/null || true
}
...
remove_project() {
    ...
    if [ -x /opt/etc/ndm/netfilter.d/100-redirect.sh ]; then
        # dns4.2.37 L-2: mangle включена в цикл явно. Раньше правила
        # mangle снимались только потому, что внутренний
        # purge_project_netfilter использует явные `-t mangle`; любое
        # будущее правило с гейтом по $table осталось бы после -remove.
        for _rm_table in mangle nat filter; do
    ...
    restore_direct_zone_upstreams /opt/etc/dnsmasq.conf
    remove_managed_file_sections /opt/etc/hosts || _rm_purge_ok=0
    rm -f /opt/etc/unblock.dnsmasq /opt/etc/unblock.dnsmasq.cidr

    # dns4.2.29 (внешний отчёт, N-01; ревизия блока из 4.2.28): симметричный
    # порядок снятия оверрайда. Контроллер уже остановлен стоп-циклом выше
    # (S99unblock stop -> utils.py --dns-stop).
    # 1) Перезапуск с УЖЕ чищеным конфигом — вытеснить из памяти записи
    #    «server=127.0.0.1#405xx» на мёртвый контроллер.
    # 2) Остановка ДО «no opkg dns-override» — зеркально порядку включения
    #    (см. ensure_dns_override: оверрайд ставится до первого запуска
    #    именно потому, что системный резолвер владеет портом 53).
    # 3) Если оверрайд НЕ снялся: чищеный конфиг не имеет ни одного
    #    дефолтного апстрима (только зонные строки), а системный резолвер
    #    на :53 не вернётся — без фолбэка резолвинг принципиально нерабочий.
    #    Дописываются публичные серверы и возвращается резолвер на :53.
    if [ -x /opt/etc/init.d/S56dnsmasq ]; then
        /opt/etc/init.d/S56dnsmasq restart >/dev/null 2>&1 || \
            warn "⚠️ dnsmasq не перезапущен с очищенным конфигом"
        ...
        if command -v ndmc >/dev/null 2>&1 \
            ...
            if ndmc -c "show running-config" 2>/dev/null \
                ...
                {
                    echo ''
                    echo '# KeenZOO fallback: opkg dns-override не снялся'
                    echo '# при удалении проекта. Резолвинг обеспечен'
                    echo '# публичными серверами. Рекомендуется: system reboot'
                    echo 'server=1.1.1.1'
                    echo 'server=8.8.8.8'
                } >> /opt/etc/dnsmasq.conf 2>/dev/null || true
                /opt/etc/init.d/S56dnsmasq start >/dev/null 2>&1 || true
                if command -v nslookup >/dev/null 2>&1; then
                    nslookup keenetic.com 127.0.0.1 >/dev/null 2>&1 || \
                        warn "⚠️ контрольный DNS-запрос не прошёл"
                fi
                warn "⚠️ opkg dns-override не снят — резолвинг оставлен на 1.1.1.1/8.8.8.8"
                warn "   Рекомендуется перезагрузка: система -> reboot"
            fi
        fi
    fi
...
def clean_cron(text):
    # dns4.2.19: лог unblock_update.sh больше не растёт бесконтрольно —
    # каноническая форма задачи ограничивает запись хвостом (tail -c
    # 524288). Старые строки нормализуем до этой формы при миграции.
    legacy_cap = '/opt/bin/unblock_update.sh >>/tmp/unblock_update.log 2>&1'
    capped = '/opt/bin/unblock_update.sh 2>&1 | tail -c 524288 >> /tmp/unblock_update.log'
    result = []
    for line in text.splitlines(keepends=True):
        fields = line.split()
        if not fields or line.lstrip().startswith('#'):
            result.append(line); continue
        if legacy_cap in line:
            line = line.replace(legacy_cap, capped)
        body = line.split('#', 1)[0]
...
KEENZOO_JOBS = [
    (lambda body: 'unblock_update.sh' in body and 'REBUILD=1' not in body,
     '00 06 * * * root /opt/bin/rotate_logs.sh >/dev/null 2>&1; /opt/bin/unblock_update.sh 2>&1 | tail -c 524288 >> /tmp/unblock_update.log; /opt/bin/rotate_logs.sh >/dev/null 2>&1'),
    (lambda body: 'unblock_update.sh' in body and 'REBUILD=1' in body,
     '00 03 * * 3 root /opt/bin/rotate_logs.sh >/dev/null 2>&1; REBUILD=1 /opt/bin/unblock_update.sh 2>&1 | tail -c 524288 >> /tmp/unblock_update.log; /opt/bin/rotate_logs.sh >/dev/null 2>&1'),
    (lambda body: 'check_updates.sh' in body,
     '40 3 * * * root /opt/bin/check_updates.sh >/dev/null 2>&1'),
...
# Presence is checked ONLY in real cron files. updates also holds the
# migrated bot_config.py, whose dictionary literals mention the same
# /opt/bin/*.sh names (paths/payload maps); substring matching them would
# fake a "job exists" and the canonical jobs would never be appended.
active_lines = [line for path in cron_paths
                for line in updates.get(path, '').splitlines()
...
repair_installed_project() {
    ...
    REPAIR_LOCK_ACQUIRED=0
    if [ "$_ri_ok" = 1 ]; then
        # rc захватывается через `cmd || rc=$?`: в ветке `if ! cmd; then`
        # $? уже отражает инвертированное условие (0 при падении), а не код
        # команды — поэтому сравнение с EX_TEMPFAIL было бы недостижимо.
        _lu_rc=0
        /opt/bin/unblock_update.sh || _lu_rc=$?
        # rc=75 (EX_TEMPFAIL) — штатная временная недоступность DNS,
        # а не ошибка транзакции: повтор по крону завтра в 06:00.
        if [ "$_lu_rc" -eq 75 ]; then
            echo "⏳ Обновление списков отложено (EX_TEMPFAIL): DNS временно недоступен; повтор по расписанию"
        elif [ "$_lu_rc" -ne 0 ]; then
            _ri_ok=0
            warn "❌ Не завершено обновление списков (rc=$_lu_rc); DNS baseline сохраняется через rollback"
        fi
    fi
...
detect_endian() {
    ...
    # Баг 5 фикс: на «голой» прошивке Keenetic утилиты `od` нет (она
    # приходит с coreutils), поэтому добавлен запасной зонд через
    # `hexdump` (есть в BusyBox); 6-й байт ELF (e_ident[EI_DATA]):
    ...
    if [ -n "$_de_probe" ]; then
        ...
        if have_cmd od; then
            ...
            _de_b="$(dd if="$_de_probe" bs=1 skip=5 count=1 2>/dev/null | hexdump -e '1/1 "%d"')"
        elif have_cmd busybox \
            && busybox hexdump -e '1/1 "%d"' /dev/null >/dev/null 2>&1; then
            # Полировка 4.2.26: на минималистичных прошивках симлинка
            # /usr/bin/hexdump может не быть, хотя апплет вкомпилирован
            # в /bin/busybox; пробуем прямой вызов с проверкой, что
            # сборка понимает формат (не все понимают -e).
            _de_b="$(dd if="$_de_probe" bs=1 skip=5 count=1 2>/dev/null | busybox hexdump -e '1/1 "%d"')"
        fi
        case "$_de_b" in
            1) printf 'le\n'; return 0 ;;
            2) printf 'be\n'; return 0 ;;
    ...
    # Баг 5 фикс: ядро MIPS пишет в /proc/cpuinfo строку вида
    # «byteorder : little», которую прежний шаблон «little.endian»
    # не матчил; принимаем обе формы.
    if grep -qiE 'little[-_ .]?endian|byteorder[[:space:]]*:[[:space:]]*little' \
        /proc/cpuinfo 2>/dev/null; then
        printf 'le\n'; return 0
    fi
    if grep -qiE 'big[-_ .]?endian|byteorder[[:space:]]*:[[:space:]]*big' \
        /proc/cpuinfo 2>/dev/null; then
        printf 'be\n'; return 0
    fi
...
RESOLV_BACKUP=""
# dns4.2.32 (статический аудит, F-07): состояние исходного резолвера —
# существовал ли файл и был ли симлинком — чтобы восстановить его на
# ЛЮБОМ выходе (die/сигнал/штатный), а не только на штатном.
RESOLV_EXISTED=0
RESOLV_LINK=""
RESOLV_MODIFIED=0
# dns4.2.35 (ре-аудит, R-10): при записи через симлинк переписывается
# ФАЙЛ-ЦЕЛЬ; сохраняем и возвращаем не только саму ссылку, но и её
# содержимое, иначе исходные nameserver теряются.
RESOLV_LINK_TARGET_BACKUP=""
...
ensure_dns() {
    ...
    #    восстанавливается в restore_dns() на всех путях выхода
    #    (dns4.2.32, F-07).
    if [ "$RESOLV_MODIFIED" != 1 ]; then
        if [ -L /etc/resolv.conf ]; then
            RESOLV_EXISTED=1
            RESOLV_LINK="$(readlink /etc/resolv.conf 2>/dev/null || true)"
            if [ -n "$RESOLV_LINK" ] && [ -e "$RESOLV_LINK" ]; then
                RESOLV_LINK_TARGET_BACKUP="$(mktemp /tmp/resolv.target.XXXXXX)"
                cat "$RESOLV_LINK" > "$RESOLV_LINK_TARGET_BACKUP" 2>/dev/null \
                    || RESOLV_LINK_TARGET_BACKUP=""
            fi
...
restore_dns() {
    [ "$RESOLV_MODIFIED" = 1 ] || return 0
    if [ -n "$RESOLV_LINK" ]; then
        # dns4.2.35 (ре-аудит, R-10): возвращаем и цель ссылки, и её
        # содержимое — запись через симлинк переписала файл-цель.
        if [ -n "$RESOLV_LINK_TARGET_BACKUP" ] \
                && [ -f "$RESOLV_LINK_TARGET_BACKUP" ]; then
            cat "$RESOLV_LINK_TARGET_BACKUP" > "$RESOLV_LINK" 2>/dev/null || true
            rm -f "$RESOLV_LINK_TARGET_BACKUP" 2>/dev/null || true
        fi
        rm -f /etc/resolv.conf 2>/dev/null || true
        ln -s "$RESOLV_LINK" /etc/resolv.conf 2>/dev/null || true
    elif [ "$RESOLV_EXISTED" = 1 ]; then
        if [ -n "$RESOLV_BACKUP" ] && [ -f "$RESOLV_BACKUP" ]; then
            cat "$RESOLV_BACKUP" > /etc/resolv.conf 2>/dev/null || true
        fi
    else
        # Исходного файла не было — подмена обязана исчезнуть.
        rm -f /etc/resolv.conf 2>/dev/null || true
    fi
...
pick_iptables() {
    ...
    # dns4.2.21 (A3 C1): /tmp world-writable — кандидаты iptables из /tmp
    # исключены (hijack выбранного бинаря); symlink отклоняется.
    ...
    # dns4.2.21 (A3 C1): glob по /tmp исключён по той же причине.
    for _c in /usr/sbin/iptables* /sbin/iptables* /bin/iptables*; do
        [ -e "$_c" ] || continue
...
# dns4.2.27 (внешний аудит, D-04): префлайт свободного места перед
# пакетным шагом — на Keenetic с флеш 16 МБ opkg может исчерпать /opt
# посреди установки. Жёсткий отказ ниже 8 МБ (пакетный набор заведомо
# не поместится), предупреждение ниже 50 МБ.
_db_avail_kb="$(df -k /opt 2>/dev/null | awk 'NR==2 {print $4}')"
case "$_db_avail_kb" in
    ''|*[!0-9]*) ;;
    *)
        if [ "$_db_avail_kb" -lt 8192 ]; then
            die "Мало места на /opt: ${_db_avail_kb} КБ — установка пакетов невозможна (нужно ~8 МБ)"
        elif [ "$_db_avail_kb" -lt 51200 ]; then
            warn "⚠️ Мало места на /opt: ${_db_avail_kb} КБ — пакеты могут установиться не все"
        fi
        ;;
esac
...
if ! ipt_is_capable "$IPT_BIN"; then
    ...
    _ipt_fw=""
    # dns4.2.21 (A3 C1): /tmp исключён (world-writable hijack).
    for _c in /usr/sbin/iptables /sbin/iptables /bin/iptables \
        /usr/bin/iptables /usr/local/sbin/iptables; do
        [ -x "$_c" ] && [ ! -L "$_c" ] || continue
        if ipt_is_capable "$_c"; then
            _ipt_fw="$_c"
            break
...
# opt/bin/update_protocols.sh (автономность деплоя, A3 H4) — ЛЮБУЮ правку вносить
...
_stop_project_xray() {
    ...
    for _spx_pid in /proc/[0-9]*; do
        ...
        _spx_cmd="$(tr '\000' ' ' < "$_spx_pid/cmdline" 2>/dev/null || true)"
        case "$_spx_cmd" in
...
# dns4.2.33/4.2.35 (аудиты, F-01/R-03): специальные зоны живут в
# управляемом блоке «BEGIN/END KeenZOO special zones» и направляются на
# петлевую фасаду контроллера (список резолверов зон — в utils.py,
# V4_ZONE_SERVERS). Апгрейд: удаляются ВСЕ старые зонные строки —
# прямые апстримы прежних установок, дубли (у /lib было четыре строки),
# остатки вне блока — и ставится канонический блок. Идемпотентно:
# повторные прогоны ничего не меняют.
if [ -f /opt/etc/dnsmasq.conf ]; then
    _kz_mig_tmp="$(mktemp /tmp/dnsmasq.mig.XXXXXX)"
    if awk '
        $0 == "# BEGIN KeenZOO special zones" { skip=1; next }
        $0 == "# END KeenZOO special zones" { skip=0; next }
        /^server=\/(lib|emc|coin|bazar|geek|libre|bbs|chan|cyb|indy|neo|null|pirate)\// { next }
        !skip { print }
    ' /opt/etc/dnsmasq.conf > "$_kz_mig_tmp" 2>/dev/null; then
        cat "$_kz_mig_tmp" > /opt/etc/dnsmasq.conf 2>/dev/null || true
    fi
    rm -f "$_kz_mig_tmp" 2>/dev/null || true
    # Блок ставится заново, если его нет ИЛИ содержимое ушло от
    # канонических 13 строк контроллера (ручная правка внутри блока).
    _kz_zone_cnt="$(grep -c '^server=/.*/127\.0\.0\.1#40512$' \
        /opt/etc/dnsmasq.conf 2>/dev/null || true)"
    if ! grep -q '^# BEGIN KeenZOO special zones$' /opt/etc/dnsmasq.conf \
            || [ "${_kz_zone_cnt:-0}" -ne 13 ]; then
        if grep -q '^# BEGIN KeenZOO special zones$' /opt/etc/dnsmasq.conf; then
            _kz_mig_tmp="$(mktemp /tmp/dnsmasq.mig.XXXXXX)"
            if awk '
                $0 == "# BEGIN KeenZOO special zones" { skip=1; next }
                $0 == "# END KeenZOO special zones" { skip=0; next }
        ...
        {
            printf '%s\n' '# BEGIN KeenZOO special zones'
            for _kz_zone in \
                lib emc coin bazar geek libre bbs chan cyb indy neo null pirate; do
                printf 'server=/%s/127.0.0.1#40512\n' "$_kz_zone"
            done
            printf '%s\n' '# END KeenZOO special zones'
        } >> /opt/etc/dnsmasq.conf 2>/dev/null || true
        echo "✅ Специальные зоны направлены через контроллер (управляемый блок)"
    else
        echo "✅ Специальные зоны: управляемый блок уже на месте"
    fi
fi
...
# dns4.2.37 M-6: хук требует аргумент `start` и без него молча выходит —
# прежний вызов без аргумента был пустышкой и наборы создавались только
# событием монтирования накопителя.
if [ -x /opt/etc/ndm/fs.d/100-ipset.sh ]; then
    /opt/etc/ndm/fs.d/100-ipset.sh start >/dev/null 2>&1 || true
fi
...
if [ -x /opt/bin/unblock_update.sh ]; then
    ...
    if /opt/bin/unblock_update.sh; then
        ...
        _lu_rc=$?
        # rc=75 (EX_TEMPFAIL) — отложено, а не ошибка: повтор по крону.
        if [ "$_lu_rc" -eq 75 ]; then
            echo "⏳ Наполнение списков отложено (EX_TEMPFAIL): DNS временно недоступен; повтор по расписанию"
        else
            warn "⚠️ unblock_update.sh вернул ошибку (rc=$_lu_rc)"
        fi
    fi
fi
...
# Правила перехвата применяются прогонами по таблицам: прогон
# «table=nat» применяет nat (TCP/DNS) И команды «-t mangle» (TPROXY) —
# mangle-секции хука не гейтятся по таблице; прогон «table=filter»
# применяет filter (доступ к веб-панели). Ни один rc не скрывается:
...
if [ -x /opt/etc/ndm/netfilter.d/100-redirect.sh ]; then
    ...
    # dns4.2.32 (статический аудит, F-03): наличие TPROXY проверяется
    # по каждому сервису отдельно (по имени его ipset в правиле):
    # прежний общий счётчик правил маскировал отсутствие правила для
    # одного из сервисов наличием правила другого.
    _tp_missing=""
    if service_ready S24xray; then
        $IPT_BIN -t mangle -S PREROUTING 2>/dev/null             | grep 'TPROXY' 2>/dev/null | grep -q 'unblockvless'             || _tp_missing="${_tp_missing} vless"
    fi
    if service_ready S57hysteria; then
        $IPT_BIN -t mangle -S PREROUTING 2>/dev/null             | grep 'TPROXY' 2>/dev/null | grep -q 'unblockhysteria'             || _tp_missing="${_tp_missing} hysteria"
    fi
    if [ -n "$_tp_missing" ]; then
        NETFILTER_OK=0
        warn "⚠️ нет TPROXY-правил для:${_tp_missing}"
        warn "   UDP этих сервисов в туннель не идёт. Возможная причина —"
        warn "   деградация в режим только TCP (ядро без TPROXY/xt_socket/xt_set)."
    fi
...
# dns4.2.35 (ре-аудит, R-01): маркер установки заставлял хук СНИМАТЬ
# (а не ставить) цепочку закрытия немаркированного DNS во всех
# прогонах этого шага; после удаления маркера в EXIT-трапе хук сам
# никто не зовёт. Снимаем маркер сейчас и делаем контрольный прогон:
# по завершении деплоя гард обязан стоять.
dns_v4_guard_active() {
    for _dga_chain in KZ_DNS_V4_FILTER KZ_DNS_V4_FILTER_NEW; do
        $IPT_BIN -C OUTPUT -p tcp --dport 53 -j "$_dga_chain" >/dev/null 2>&1 \
            && $IPT_BIN -C OUTPUT -p udp --dport 53 -j "$_dga_chain" >/dev/null 2>&1 \
            && return 0
    done
    return 1
}
rm -f "$DEPLOY_INSTALL_MARKER"
if [ -x /opt/etc/ndm/netfilter.d/100-redirect.sh ]; then
    _nf_log="$(mktemp /tmp/redirect.final.XXXXXX)"
    if type=iptable table=filter /opt/etc/ndm/netfilter.d/100-redirect.sh \
            >"$_nf_log" 2>&1 && dns_v4_guard_active; then
        echo "✅ DNSv4 гард активен (немаркированный DNS на :53 закрыт)"
    else
        NETFILTER_OK=0
        warn "⚠️ DNSv4 гард не встал — немаркированный DNS на :53 НЕ закрыт"
        sed 's/^/     /' "$_nf_log" | tail -5 >&2
    fi
    rm -f "$_nf_log"
fi
...
if [ "$TP_CNT" -gt 0 ] 2>/dev/null; then
    ...
    if ! $IPT_BIN -j TPROXY --help >/dev/null 2>&1; then
        ...
        # dns4.2.28 (внешний отчёт, P-02): прежняя подсказка звала хук
        # без table=, а его диспатч в этом случае делает «*) exit 0» —
        # пользователь получал «успех» без единого правила.
        echo "       type=iptable table=nat    /opt/etc/ndm/netfilter.d/100-redirect.sh"
        echo "       type=iptable table=filter /opt/etc/ndm/netfilter.d/100-redirect.sh"
        echo "       (прогон с table=nat дополнительно применяет mangle/TPROXY;"
        echo "        без параметра table= хук осознанно ничего не делает)"
    fi
fi

# dns4.2.35 (ре-аудит, R-06): хук пишет построчные итоги применения
# TPROXY на КАЖДОМ интерфейсе; успешный один интерфейс больше не
# маскирует пропуск на другом.
if [ -s /opt/etc/unblock/.tproxy_state ]; then
    _tps_fail="$(grep '^fail ' /opt/etc/unblock/.tproxy_state 2>/dev/null || true)"
    _tps_deg="$(grep '^degraded ' /opt/etc/unblock/.tproxy_state 2>/dev/null || true)"
    _tps_ok="$(grep -c '^ok ' /opt/etc/unblock/.tproxy_state 2>/dev/null || true)"
    if [ -n "$_tps_fail" ]; then
        echo "    ⚠️ TPROXY отклонено на отдельных интерфейсах (см. /opt/etc/unblock/.tproxy_state):"
        printf '%s\n' "$_tps_fail" | sed 's/^fail /       /'
    fi
    if [ -n "$_tps_deg" ]; then
        echo "    ⚠️ Ядерная деградация TPROXY: $(printf '%s' "$_tps_deg" | sed 's/^degraded [^ ]* [^ ]* [^ ]* //')"
    fi
    if [ -z "$_tps_fail" ] && [ -z "$_tps_deg" ]; then
        echo "    ✅ TPROXY применён на всех интерфейсах (${_tps_ok:-0} пар)"
    fi
fi
...
# dns4.2.24 (аудит A6 N4): подсказка про доступ по имени роутера.
# Проверка Origin доверяет именам из списка router_hostnames в
# /opt/etc/bot/bot_config.py — добавьте туда своё имя, если открываете
# панель не по IP (например, по собственному DNS-имени роутера).
echo "         доступ по имени: имя должно быть в router_hostnames"
echo "         (/opt/etc/bot/bot_config.py, по умолчанию my.keenetic.net)"
