...
remove_project() {
    ...
    # dns4.2.28 (внешний отчёт, P-05): DNS-стек проекта не должен
    # оставаться живым после отключения. Контроллер уже остановлен
    # стоп-циклом выше (S99unblock stop -> utils.py --dns-stop).
    # Дальше: перезапуск dnsmasq с УЖЕ чищеным конфигом (в памяти ещё
    # висят «server=127.0.0.1#405xx» на мёртвый контроллер), затем
    # возврат порта 53 прошивке через «no opkg dns-override». Остановка
    # dnsmasq — только после подтверждённого снятия оверрайда, иначе
    # роутер остался бы вообще без резолвера. Всё мягко: неудача не
    # прерывает удаление.
    if [ -x /opt/etc/init.d/S56dnsmasq ]; then
        ...
        if command -v ndmc >/dev/null 2>&1 \
            ...
            if ndmc -c "show running-config" 2>/dev/null \
                | grep -q "opkg dns-override"; then
                warn "⚠️ opkg dns-override не снят — dnsmasq оставлен с очищенным конфигом"
            else
