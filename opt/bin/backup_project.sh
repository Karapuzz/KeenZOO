...
# dns4.2.21 (A3 H5): BACKUP_DIR — только реальный каталог внутри /tmp
# или /opt: realpath-allowlist против записи через symlink за пределы.
_bk_resolve() {
    _bk_in="$1"
    [ -L "$_bk_in" ] && return 1
    if command -v realpath >/dev/null 2>&1; then
        realpath -m "$_bk_in" 2>/dev/null
    elif command -v readlink >/dev/null 2>&1; then
        readlink -f "$_bk_in" 2>/dev/null || printf '%s\n' "$_bk_in"
    else
        printf '%s\n' "$_bk_in"
    fi
}
_bk_resolved="$(_bk_resolve "$BACKUP_DIR")" || {
    echo "ОТКАЗ: BACKUP_DIR=$BACKUP_DIR — symlink не допускается" >&2; exit 1; }
case "$_bk_resolved" in
    /tmp|/tmp/*|/opt|/opt/*) BACKUP_DIR="$_bk_resolved" ;;
    *) echo "ОТКАЗ: BACKUP_DIR=$BACKUP_DIR → $_bk_resolved вне /tmp|/opt" >&2; exit 1 ;;
esac
