...
def _ensure_xray_mux_udp443(parsed):
    """
    Гарантирует прохождение QUIC (UDP/443) в туннель.

    Без mux-блока с разрешением при flow=xtls-rprx-vision ядро
    отбрасывает UDP/443 ещё до отправки, не оставляя следов на
    проектном loglevel=error: клиенты скатываются на TCP/HTTPS, и
    диагноз в логах не находится. Проект заворачивает в обход весь
    UDP с LAN (dokodemo-inbound UDP + TPROXY), поэтому проектная
    политика «весь UDP в туннель» сильнее пользовательских значений
    mux в vless-outbound с vision: четыре ключевых поля приводятся к
    проектному блоку KEENZOO_XRAY_MUX_UDP443, прочие поля mux
    пользователя сохраняются. Функция идемпотентна.
    """
    if not isinstance(parsed, dict):
        return parsed
    outbounds = parsed.get('outbounds')
