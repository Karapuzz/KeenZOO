...
# dns4.2.33 (статический аудит, F-01): резолверы специальных зон
# (альтернативные TLD). Запросы к ним идут ЧЕРЕЗ КОНТРОЛЛЕР маркированным
# TCP-сокетом (через туннель), а не напрямую: немаркированный внешний
# DNS на :53 закрыт фильтром. Состав синхронен с dnsmasq.conf, где эти
# зоны направлены на петлевую фасаду контроллера (127.0.0.1#40512).
V4_ZONE_SERVERS = {
    'lib': ('65.21.1.106', '95.216.99.249',
            '94.247.43.254', '195.10.195.195'),
    'emc': ('95.216.99.249',),
    'coin': ('95.216.99.249',),
    'bazar': ('95.216.99.249',),
    'geek': ('95.216.99.249',),
    ...
    'neo': ('95.216.99.249',),
    'null': ('95.216.99.249',),
    'pirate': ('95.216.99.249',),
}


def _v4_zone_for(name):
    """Имя зоны (без точки), если запрос к специальной зоне."""
    try:
        text = bytes(name).rstrip(b'.').decode('ascii').lower()
    except (UnicodeDecodeError, AttributeError,
            TypeError):
        return None
    if not text:
        return None
    if text in V4_ZONE_SERVERS:
        return text
    for zone in V4_ZONE_SERVERS:
...
class DNSPolicyV4:
    ...
    def commit(self, mode, target):
        with self.guard:
            ...
            # dns4.2.37 H-1: длинный гейт (recovery_interval) после провала
            # СОХРАНЁН как антидребезг КЛИЕНТСКИХ ошибок — без него каждый
            # сбой запроса долбил бы пробы каждые 30 с при отсутствующем WAN
            # (регрессия «unavailable retry is bounded»). Исключение из
            # гейта для сетевых событий (--dns-event) и переранжиров сделано
            # в tick(): они обрабатываются сразу, не ожидая 5 минут.
    ...
    def forward(self, query, tcp=False):
        ...
        try:
            question = v4_question(query)
            if query[2] & 0xf8:
                raise ValueError('invalid query opcode')
        except (ValueError, IndexError):
            return v4_fail(query)
        # dns4.2.33/4.2.35 (аудиты, F-01, R-04): специальные зоны
        # резолвит только контроллер; путь к резолверам зон зависит
        # от режима политики (см. forward_zone): туннель — через
        # туннель, режимы без туннеля — разрешённым прямым путём
        # (аварийная метка). Прямой немаркированный DNS закрыт.
        # question = ((имя, тип-класс), смещение)
        _fwd_zone = _v4_zone_for(question[0][0])
        if _fwd_zone:
            return self.forward_zone(query, _fwd_zone)
        with self.guard:
    ...
    def forward_zone(self, query, zone):
        # Публичные резолверы альтернативные TLD не знают — зоны
        # ходят только к резолверам зон (V4_ZONE_SERVERS). Выбор пути
        # по режиму политики:
        #   TUNNEL_DNS  — через туннель, метка активного кандидата;
        #   LOCAL_DNSSEC / EMERGENCY_DNS — разрешённым ПРЯМЫМ путём
        #     (аварийная метка: для неё в фильтре есть RETURN), как и
        #     до закрытия немаркированного DNS, но строго к
        #     закреплённым адресам резолверов зон;
        #   нет рабочего резолвера — SERVFAIL, как и весь трафик.
        with self.guard:
            target = self.active
            mode = self.mode
            fresh = (_v4_time.monotonic()
                     - self.last_heartbeat <= 180)
            self.query_count += 1
        if not target or not fresh \
                or mode == 'DNS_UNAVAILABLE':
            self.request_check()
            return v4_fail(query)
        if mode == 'TUNNEL_DNS':
            _fz_mark = target[3]
            if _fz_mark not in V4_MARKS.values():
                return v4_fail(query)
        else:
            _fz_mark = V4_EMERGENCY_MARK
        # dns4.2.35 (ре-аудит): общий бюджет зоны ~4 с, иначе при
        # недоступных резолверах запрос занимает слоты семафора на
        # срок до «число апстримов × 2 с». Остаток бюджета идёт в
        # таймаут каждой попытки.
        _fz_deadline = _v4_time.monotonic() + 4.0
        for host in V4_ZONE_SERVERS[zone]:
            _fz_left = _fz_deadline - _v4_time.monotonic()
            if _fz_left < 0.2:
                break
            try:
                response = self.exchange(
                    query, host, 53, True, _fz_mark,
                    timeout=min(2.0, _fz_left))
                v4_answer(response, query)
                return response
            except (OSError, ValueError, IndexError):
                continue
    ...
    def tick(self):
        ...
        with self.guard:
            # dns4.2.37 H-1: сетевые события (--dns-event от WAN-воркера)
            # и запросы переранжирования проходят БЕЗ ожидания
            # next_event — иначе после DNS_UNAVAILABLE (когда
            # next_event = now + recovery_interval) поднявшийся туннель
            # игнорировался до пяти минут. Клиентские ошибки по-прежнему
            # антидребезжатся гейтом времени.
            if reason is None and (self.pending_network or self.pending_failure or self.pending_rerank) \
                    and (self.pending_network or self.pending_rerank or now >= self.next_event):
...
def v4_main(command):
    ...
    if command == '--dns-stop':
        ...
        return 0
    if _V4Path('/opt/etc/unblock/.disabled').exists():
        # dns4.2.23 (аудит A5 #8): отказ был молчаливым — оператор в
        # терминале видел только «did not start» без причины.
        print('DNSv4 disabled: присутствует /opt/etc/unblock/.disabled'
              ' — контроллер не запускается (снимите маркер для '
              'возобновления)', file=_v4_sys.stderr)
        return 1
    if command == '--dns-start':
        ...
        with log.open('ab') as out:
            # dns4.2.27 (внешний аудит, U-05): PYTHONDONTWRITEBYTECODE
            # выставляется явно — дочерний демон не должен зависеть от
            # экспорта переменной в родительском стартовом скрипте и
            # писать __pycache__ на флеш.
            _v4_env = {k: v for k, v in _v4_os.environ.items() if k not in ('KEENZOO_UPDATE_LOCK_HELD', 'KEENZOO_LOCK_DIR', 'KEENZOO_DNS_STAGE', 'DNS_HEALTH_LOG')}
            _v4_env['PYTHONDONTWRITEBYTECODE'] = '1'
    ...
    with _v4_os.fdopen(lock_fd, 'a') as lock:
        ...
        # dns4.2.37 П-8: 24 слота хватало, пока за фасадом стоял один
        # апстрим; при пробах/рейтинге нескольких кандидатов и всплесках
        # списка очередь на семафоре становилась узким местом. 64 слота
        # на MIPS-роутере — умеренная цена за отсутствие искусственного
        # ограничения параллелизма клиентских запросов.
        slots = _v4_thread.BoundedSemaphore(64)
...
def signal_handler(sig, _frame):
    # dns4.2.37 L-6: frame не используется (сигнал обрабатывается по имени).
    log_error(
...
def check_restart(bot):
    ...
    if os.path.exists(chat_id_path):
        try:
            ...
            # dns4.2.30 (полный аудит, П1): пустой или битый файл
            # (потеря питания в момент записи) не должен навсегда
            # ломать запуск бота: раньше необработанный ValueError
            # убивал процесс до polling, файл не удалялся — и каждый
            # следующий старт падал снова. Паритет с
            # generator._lock_is_live.
            log_error(
                f"check_restart: повреждён {chat_id_path}: {e}")
            try:
...
def _lint_xray_transports(tmp_path):
    """
    Линт соответствия «транспорт ↔ блок настроек» после успешного
    'xray -test'.

    Ядро молча принимает конфиг с 'network': 'xhttp' без блока
    xhttpSettings (или с устаревшим splithttpSettings): проверка
    проходит, а в рантайме транспорт получает path '/' и mode 'auto'
    вместо значений из ключа, и сервер рвёт соединение без внятной
    диагностики. Обнаружить это статически возможно только сверкой
    транспорта с настройками — здесь она и выполняется.

    Результат — предупреждение в лог; запись конфига НЕ блокируется,
    чтобы линт не ломал обновление рабочих конфигураций.
    """
    try:
        with open(tmp_path, 'r', encoding='utf-8') as cfg_file:
            parsed = json.load(cfg_file)
    except (OSError, ValueError):
        return
    ...
    for ob in outbounds:
        ...
        if stream.get('network') != 'xhttp':
            continue
        tag = ob.get('tag') or ob.get('protocol') or '?'
        if 'xhttpSettings' not in stream:
            log_error(
                "Xray lint: outbound '%s' — network=xhttp без "
                "xhttpSettings: ядро принимает конфиг молча, но "
                "транспорт уходит в значения по умолчанию "
                "(path=/, mode=auto)" % tag)
        if 'splithttpSettings' in stream:
            log_error(
                "Xray lint: outbound '%s' — устаревший "
                "splithttpSettings при network=xhttp: блок игнорируется "
                "ядром, используйте xhttpSettings" % tag)
...
def _preflight_xray(tmp_path):
    ...
    for cmd in (
        ...
        try:
            ...
            continue
        if res.returncode == 0:
            # Конфиг ядром принят. Отдельно сверяем транспортные блоки:
            # ядро молча разрешает несовпадение (см. _lint_xray_transports).
            _lint_xray_transports(tmp_path)
            return
...
def _preflight_hysteria(tmp_path):
    ...
    # dns4.2.36 (аудит, KZ-09): одних ключей недостаточно — проверяем
    # сами адреса слушателей и совпадение порта с перехватом трафика
    # (правила фаервола ведут на порт из bot_config).
    for _pf_key in ('tcpRedirect', 'udpTProxy'):
        _pf_listen = data[_pf_key].get('listen')
        if not isinstance(_pf_listen, str) \
                or ':' not in _pf_listen:
            raise ValueError(
                "Hysteria: %s.listen должен быть строкой «адрес:порт»"
                % _pf_key)
        _pf_port = _pf_listen.rsplit(':', 1)[-1]
        if not _pf_port.isdigit() \
                or not (1 <= int(_pf_port) <= 65535):
            raise ValueError(
                "Hysteria: %s.listen — некорректный порт: %r"
                % (_pf_key, _pf_listen))
        if int(_pf_port) != int(config.localporthysteria):
            raise ValueError(
                "Hysteria: %s.listen порт %s != %s из bot_config — "
                "перехват трафика на этот порт не настроен"
                % (_pf_key, _pf_port, config.localporthysteria))
...
def _mark_tunnel_configured(file_path):
    ...
    mapping = {
        ...
        # dns4.2.20: shadowsocks тоже маркируется — иначе guard
        # S65shadowsocks (симметричный S22/S24/S57) заблокировал бы
        # ss-redir навсегда. Правка применяется только вместе с guard.
        os.path.realpath(config.paths.get('shadowsocks_config', '')): 'shadowsocks',
    }
...
class ConfigWriter:
    ...
    def write_config(file_path,
        ...
        try:
            ...
            # dns4.2.36 (аудит, KZ-09): для туннельных конфигов держим
            # копию прежнего содержимого: если запись маркера
            # .configured упадёт ПОСЛЕ замены конфига, откатываем —
            # иначе сервис остался бы с новым конфигом, но без права
            # запуска (и без простого способа вернуться назад).
            _kz_tunnel_cfgs = (
                os.path.realpath(config.paths.get('vless_config', '')),
                os.path.realpath(config.paths.get('trojan_config', '')),
                os.path.realpath(config.paths.get('hysteria_config', '')),
                os.path.realpath(
                    config.paths.get('shadowsocks_config', '')))
            _kz_old_backup = None
            if real_path in _kz_tunnel_cfgs \
                    and os.path.exists(file_path):
                try:
                    with open(file_path, 'rb') as _kz_of:
                        _kz_old_backup = _kz_of.read()
                except OSError:
                    _kz_old_backup = None
            ...
            except Exception as _kz_marker_err:
                if _kz_old_backup is not None:
                    try:
                        _kz_fd, _kz_rb = tempfile.mkstemp(
                            dir=directory or '.', suffix='.rollback')
                        with os.fdopen(_kz_fd, 'wb') as _kz_rf:
                            _kz_rf.write(_kz_old_backup)
                            _kz_rf.flush()
                            os.fsync(_kz_rf.fileno())
                        os.replace(_kz_rb, file_path)
                    except OSError:
                        log_error(
                            "KZ-09: не удалось откатить конфиг после "
                            "сбоя маркера: " + file_path)
                log_error(
                    "Маркер .configured не записан — конфиг откачен: "
                    "%s" % _kz_marker_err)
                raise
...
def parse_vless_key(key, bot=None,
    ...
    return {
        ...
        'spx': params.get(
            'spx', ['/'])[0],
        # dns4.2.17/4.2.18: allowInsecure=1 и алиас insecure=1 — намерение
        # «самоподписанный сервер»; доставляется в render (generate_config →
        # _normalize_xray_insecure → pinnedPeerCertSha256), т.к. на Xray
        # ≥26 поле allowInsecure отсутствует и до этого молча испарялось.
        'allowInsecure': params.get(
            'allowInsecure',
            params.get('insecure', ['0']))[0],
        # dns4.2.18: хеш сертификата из ссылки (&pin= канонично; &hash= и
        # полное имя — алиасы). Hex64 по DER; base64 ядро не принимает.
        'pinnedPeerCertSha256': params.get(
            'pin',
            params.get(
                'hash',
                params.get('pinnedPeerCertSha256', [''])))[0].lower(),
        'transport': transport,
...
def parse_trojan_key(key, bot=None,
    ...
    result = {
        ...
        'host_header': params.get(
            'host', [''])[0],
        # dns4.2.17: insecure=1 — алиас allowInsecure=1 (устоявшийся в
        # vless-share). Дальше маршруты разные: vless (ядро Xray ≥26) —
        # конвертация в verifyPeerCertByName; trojan (своё ядро
        # trojan-go) — verify=false; параметр больше нигде не испаряется.
        'allowInsecure': params.get(
            'allowInsecure',
            params.get('insecure', ['0']))[0],
        'serviceName': params.get(
...
def parse_hysteria_key(key, bot=None,
    ...
    ap = [p.strip()
          ...
          if p.strip()]
    # Баг 6 фикс: раньше пустой ALPN превращался в `""`, шаблон давал
    # `"alpn": [""]`, и `_cleanup_empty_fields` удаляла ключ целиком —
    # клиент стучался по QUIC вообще без предложения ALPN, а строгие
    # серверы это отклоняют. Стандарт для hy2 — «h3»; он и подставляется,
    # когда пользователь явно не задал свой список.
    if not ap:
        ap = ['h3']
    alpn = ', '.join(
        f'"{p}"' for p in ap)
...
# dns4.2.32 (статический аудит, F-02): единая семантика правил
# маршрутизации. Прежние три независимые проверки расходились:
# has_catch_all не смотрела на ip/domain/source-селекторы, has_udp_rule
# принимала любое правило с «udp» в network (даже ограниченное), а
# поиск дефолта не считал «network» условием — из-за этого вставка
# «udp,tcp -> proxy» могла переопределить пользовательское
# «network: tcp -> direct».
_RULE_CONDITION_KEYS = (
    'domain', 'domains', 'ip', 'source', 'sourcePort',
    'protocol', 'network', 'inboundTag', 'attrs', 'user',
    'rule_set', 'geosite', 'geoip',
)


def _rule_port_unconditional(rule):
    # «0-65535» покрывает все порты и приравнивается к отсутствию.
    return rule.get('port') in (None, '', '0-65535')
...
def _ip_covers_all(ip):
    # «ip: [0.0.0.0/0]» покрывает всё и приравнивается к
    # отсутствию селектора (дефолт вида «всё остальное -> direct»).
    # dns4.2.35 (ре-аудит, R-05): семантика Xray — правило матчится
    # при совпадении ЛЮБОГО элемента списка, поэтому достаточно ОДНОЙ
    # записи, покрывающей весь диапазон:
    #   ip: ["0.0.0.0/0", "192.0.2.0/24"]  →  покрывает всё.
    if not ip:
        return True
    if isinstance(ip, str):
        return ip.strip() in _ANY_IP
    if isinstance(ip, list):
        return any(
            str(x).strip() in _ANY_IP for x in ip)
    return False


def _is_unconditional(rule):
    if not isinstance(rule, dict):
        return False
    for key in _RULE_CONDITION_KEYS:
        if not rule.get(key):
            continue
        if key == 'ip' and _ip_covers_all(rule.get('ip')):
            continue
        return False
    return _rule_port_unconditional(rule)


def _covers_all_udp(rule, proxy_tag=None):
    if not isinstance(rule, dict):
        return False
    if proxy_tag is not None \
            and rule.get('outboundTag') != proxy_tag:
        return False
    network = rule.get('network')
    if isinstance(network, str):
        nets = {n.strip() for n in network.split(',')
                if n.strip()}
    elif isinstance(network, list):
        nets = {str(n).strip() for n in network
                if str(n).strip()}
    else:
        nets = {'tcp', 'udp'}
    if 'udp' not in nets:
...
def _balancer_has_proxy(routing, balancer_tag, outbounds,
                        proxy_kinds, direct_kinds):
    # dns4.2.35 (ре-аудит, R-05): безусловное правило с
    # «balancerTag» — тоже дефолт маршрутизации. Разрешаем
    # балансировщик: его селекторы матчат теги outbound-ов
    # по префиксу; если среди выбранных есть прокси-выход,
    # политика пользователя уже покрывает трафик — своё
    # правило перед ним не ставим (не обходим балансировщик).
    ...
    for b in balancers:
        if isinstance(b, dict) \
                and b.get('tag') == balancer_tag:
            sel = b.get('selector')
            if isinstance(sel, str):
                sel = [sel]
            selectors = [str(x) for x in (sel or [])]
            break
    ...
    for ob in outbounds:
        ...
        if not any(tag.startswith(x) for x in selectors):
            continue
...
def _ensure_xray_udp_routing(parsed):
    ...
    if not has_catch_all:
        has_catch_all = any(
            isinstance(r, dict)
            and _is_unconditional(r)
            and bool(r.get('balancerTag'))
            and _balancer_has_proxy(
                routing, r.get('balancerTag'),
                outbounds, proxy_kinds, direct_kinds)
            for r in rules)

    has_udp_rule = False
    for r in rules:
        if _covers_all_udp(r, proxy_tag):
            has_udp_rule = True
            break
        if isinstance(r, dict) and _covers_all_udp(r) \
                and r.get('balancerTag') \
                and _balancer_has_proxy(
                    routing, r.get('balancerTag'),
                    outbounds, proxy_kinds, direct_kinds):
            has_udp_rule = True
    ...
    if not has_catch_all and not has_udp_rule:
        ...
        # Баг 8 фикс: правила маршрутизации матчатся СВЕРХУ ВНИЗ.
        # Прежний `rules.append` ставил наше правило после чужого
        # завершающего («всё остальное -> direct») в импортированных
        # конфигах, и оно никогда не срабатывало — весь перехваченный
        # трафик шёл мимо прокси. Но и вставка в позицию 0 неверна:
        # она переопределила бы пользовательские предметные правила
        # (домены/диапазоны -> direct). Ставим правило перед ПЕРВЫМ
        # обнаруженным завершающим правилом; если такого нет — в конец.
        # dns4.2.32 (F-02): дефолт ищем по той же семантике;
        # «network: tcp -> direct» теперь НЕ признаётся безусловным
        # и наше правило встаёт ПОСЛЕ него, не ломая политику.
        pos = len(rules)
        for i, r in enumerate(rules):
            if _is_unconditional(r):
                pos = i
                break
...
# Проектный mux-блок для vless-outbound-ов с flow=xtls-rprx-vision:
# включает ТОЛЬКО XUDP-канал для UDP/443. concurrency < 0 полностью
# отключает TCP-сторону mux (см. infra/conf/xray.go у Xray-core:
# "Concurrency < 0 completely disables mux"), поэтому TCP остаётся в
# прямом vision-режиме (splice/zero-copy) и не нагружает CPU роутера,
# а QUIC (UDP/443) перестаёт молча отбрасываться ядром
# ("XTLS rejected UDP/443 traffic" / "XUDP rejected UDP/443 traffic" —
# оба на уровне Info, которого в проектных логах нет).
KEENZOO_XRAY_MUX_UDP443 = {
    'enabled': True,
    'concurrency': -1,
    'xudpConcurrency': 8,
...
def _ensure_xray_mux_udp443(parsed):
    """
    Управляет прохождением QUIC (UDP/443) в туннель по flow-семантике.

    Действие по отношению к mux-блоку у каждого vless-outbound:

    * flow НАЧИНАЕТСЯ с xtls-rprx-vision — впрыснуть/привести к
      проектному блоку KEENZOO_XRAY_MUX_UDP443. Без mux-блока с
      разрешением ядро при vision отбрасывает UDP/443 ещё до отправки,
      не оставляя следов на проектном loglevel=error: клиенты
      скатываются на TCP/HTTPS, и диагноз в логах не находится.
      Проект заворачивает в обход весь UDP с LAN (dokodemo-inbound
      UDP + TPROXY), поэтому политика «весь UDP в туннель» сильнее
      пользовательских значений mux при vision: четыре ключевых поля
      приводятся к проектным, прочие поля пользователя сохраняются.

    * vision-flow НЕТ — проектный mux-блок снимается, если он записан
      точно константой KEENZOO_XRAY_MUX_UDP443 (оставлен от смены ключа
      vision -> не-vision). Для не-vision-путей (WS+TLS, gRPC и пр.,
      где сервер нередко sing-box/не-Xray) mux-блок давал бы регресс:
      весь UDP уходил бы в XUDP-сеанс, который нативно понимает только
      Xray-сервер, тогда как без mux родной VLESS-UDP с не-vision
      работает и на других серверах. Пользовательский mux (значения
      отличаются от проектной константы) не трогается совсем.

    Функция идемпотентна.

    Известное допущение (сознательно не правится): снятие происходит
    только при полном совпадении mux с проектной константой. Если
    пользователь сам правил mux (скажем, xudpConcurrency) при vision-
    ключе, блок уже не совпадает с константой, и при последующей смене
    ключа на не-vision он останется — на сервере не-Xray UDP тогда
    пойдёт по XUDP (для таких серверов это не рабочий транспорт).
    Это осознанный компромисс: автозатирание пользовательских значений
    mux опаснее сохранения его поведения. Рекомендация по эксплуатации —
    в чек-листе маршрутизатора: для не-vision ключа с сервером sing-box
    применить vless:// заново (guard снимет проектный блок корректно).
    """
    if not isinstance(parsed, dict):
        return parsed
    outbounds = parsed.get('outbounds')
    ...
    for ob in outbounds:
        ...
        if vision:
            if not isinstance(mux, dict):
                mux = {}
                ob['mux'] = mux
            mux.update(KEENZOO_XRAY_MUX_UDP443)
            continue
        # Не-vision: снимаем только проектный по значениям mux.
        if (isinstance(mux, dict)
                and len(mux) == len(KEENZOO_XRAY_MUX_UDP443)
                and all(mux.get(k) == v for k, v in
                        KEENZOO_XRAY_MUX_UDP443.items())):
            del ob['mux']
    return parsed
...
def generate_config(key, template_file,
    ...
    if template_file.endswith('.json'):
        ...
        if template_file == 'vless_template.json':
            ...
            parsed = _ensure_xray_mux_udp443(parsed)
            # dns4.2.18: &pin=<64-hex> в ссылке — само по себе намерение
            # закрепить сертификат; allowInsecure=1/insecure=1 без хеша —
            # TOFU-зонд. Шаблон несёт allowInsecure:false, без intent-проброса
            # параметр молча испарялся (dns4.2.17 закрыла этот flatten).
            _gci_pin = str(params.get('pinnedPeerCertSha256', '') or '')
            parsed = _normalize_xray_insecure(
                parsed,
                intent_flag=(str(params.get('allowInsecure', '')).lower()
                             in ('1', 'true', 'yes') or bool(_gci_pin)),
                intent_name=params.get('sni') or params.get('address'),
                intent_pin=_gci_pin or None)
...
def hysteria_config(key, bot=None,
    ...
    generate_config(
        ...
        bot=bot, chat_id=chat_id)
def _valid_cert_pin(value):
    """hex SHA-256 (64 символа) от DER сертификата; base64 ядро не принимает."""
    return isinstance(value, str) and bool(
        _v4_re.fullmatch(r'[0-9a-fA-F]{64}', value or ''))


def _fetch_server_cert_sha256(host, port, sni):
    """SHA-256 (hex) по DER сертификата сервера; None при недоступности.

    Один прямой TLS-коннект с SSL.CERT_NONE (trust-on-first-use): сертификат
    как раз и неизвестен, поэтому верификация здесь бессмысленна. Изолирован
    в отдельную функцию для моков в регрессии — сеть из тестов не вызывается.
    """
    try:
        import hashlib as _pin_hashlib
        import socket as _pin_socket
        import ssl as _pin_ssl
        ctx = _pin_ssl.SSLContext(_pin_ssl.PROTOCOL_TLS_CLIENT)
        ctx.check_hostname = False
        ctx.verify_mode = _pin_ssl.CERT_NONE
        with _pin_socket.create_connection(
                (host, int(port)), timeout=6) as sock:
            with ctx.wrap_socket(
                    sock, server_hostname=(sni or None)) as ssock:
                der = ssock.getpeercert(binary_form=True)
        if not der:
            return None
        return _pin_hashlib.sha256(der).hexdigest()
    except Exception:
        return None


def _outbound_endpoint(ob):
    """address/port для TOFU-зонда из settings.vnext[0]."""
    st = ob.get('settings')
    if not isinstance(st, dict):
        return None, None
    vn = st.get('vnext')
    if isinstance(vn, list) and vn and isinstance(vn[0], dict):
        return vn[0].get('address'), vn[0].get('port')
    return None, None


def _pin_helper_cmd(host, port, sni):
    return ("openssl s_client -connect %s:%s -servername %s </dev/null "
            "2>/dev/null | openssl x509 -outform DER | openssl sha256"
            % (host or '<host>', port or 443, sni or '<sni>'))


def _log_pin_conversion(pin, tag, origin, host, port, sni):
    log_error(
        "Xray: allowInsecure → pinnedPeerCertSha256 '%s…' (outbound '%s', "
        "%s): ядро ≥26 убрал allowInsecure; сертификат закреплён по SHA-256 "
        "(trust-on-first-use) — сверьте хеш: %s. После перевыпуска "
        "сертификата примените ключ заново"
        % (pin[:12], tag, origin, _pin_helper_cmd(host, port, sni)))


def _normalize_xray_insecure(parsed, intent_flag=None, intent_name=None,
                             intent_pin=None):
    """allowInsecure/pin-намерение → pinnedPeerCertSha256 (Xray ≥ 26).

    Xray ≥ 25.x убрал поле allowInsecure (hard-fail проверки конфига:
    «migrated to pinnedPeerCertSha256 / verifyPeerCertByName»). Единственный
    рабочий аналог для самоподписанного сервера — pinnedPeerCertSha256
    (hex SHA-256 по DER): verifyPeerCertByName сверяет имя ПОВЕРХ
    CA-проверки и самоподписанный сертификат по-прежнему отклоняет.

    Приоритеты в tlsSettings (только при streamSettings.security == 'tls';
    reality/none не трогаются — там tlsSettings очисткой снимается, и
    конвертация порождала ложные записи в журнале):
      1. Нативный pinnedPeerCertSha256 (пользователь задал явно) — принят,
         недействительный allowInsecure просто снимается.
      2. Явный verifyPeerCertByName пользователя — принят, allowInsecure
         снимается (поле мёртво на новом ядре), в журнал — одна честная
         строка.
      3. Хеш из входа (intent_pin: параметр ссылки &pin=/&hash=/
         &pinnedPeerCertSha256=, sing-box tls.pin_sha256) — hex64 или отказ.
      4. TOFU-зонд к address:port с SNI (см. _fetch_server_cert_sha256);
         недостижим → ValueError с openssl-командой для ручного хеша.
    """
    if not isinstance(parsed, dict):
        return parsed
    outbounds = parsed.get('outbounds')
    if not isinstance(outbounds, list):
        outbounds = []
    if intent_pin and not _valid_cert_pin(intent_pin):
        raise ValueError(
            "Пин сертификата: ожидается 64 hex-символа SHA-256 по DER; "
            "base64 ядром Xray не принимается. Получить: %s"
            % _pin_helper_cmd(None, None, intent_name))
    pin_from_intent = (intent_pin or '').lower() or None
    converted = 0
    tls_seen = 0
    for ob in outbounds:
        if not isinstance(ob, dict):
        ...
        tls_seen += 1
        tag = ob.get('tag') or ob.get('protocol') or '?'
        host, port = _outbound_endpoint(ob)
        sni = tls.get('serverName') or intent_name
        if _valid_cert_pin(tls.get('pinnedPeerCertSha256')):
            tls['pinnedPeerCertSha256'] = tls[
                'pinnedPeerCertSha256'].lower()
            if tls.pop('allowInsecure', None):
                log_error(
                    "Xray: allowInsecure снят у outbound '%s' — на ядре ≥26 "
                    "поле мертво; действует ваш pinnedPeerCertSha256" % tag)
            continue
        if bool(tls.get('allowInsecure')) and tls.get('verifyPeerCertByName'):
            tls.pop('allowInsecure', None)
            log_error(
                "Xray: allowInsecure снят у outbound '%s' — на ядре ≥26 "
                "поле мертво; действует ваш verifyPeerCertByName '%s'"
                % (tag, tls['verifyPeerCertByName']))
            continue
        want = bool(tls.get('allowInsecure')) or bool(
            intent_flag and not tls.get('pinnedPeerCertSha256'))
        if not want:
            continue
        pin = pin_from_intent
        origin = 'хеш из входа'
        if not pin:
            pin = _fetch_server_cert_sha256(host, port, sni) if host else None
            origin = 'TOFU-зонд'
        if not pin:
            raise ValueError(
                "Xray ≥26 не поддерживает allowInsecure; сертификат сервера "
                "недоступен для автоматического пиннинга (прямое подключение "
                "к %s:%s не удалось). Укажите хеш вручную (&pin= в ссылке "
                "или поле pinnedPeerCertSha256): %s"
                % (host, port, _pin_helper_cmd(host, port, sni)))
        tls.pop('allowInsecure', None)
        tls['pinnedPeerCertSha256'] = pin
        converted += 1
        _log_pin_conversion(pin, tag, origin, host, port, sni)
    if intent_flag and converted == 0 and tls_seen == 0:
        log_error(
            "Xray: запрошен allowInsecure/insecure=1, но TLS-outbound "
            "не найден (security=reality/none): под reality параметр не "
            "имеет смысла и проигнорирован")
    return parsed


def _geo_asset_ref(entry):
    # dns4.2.35 (ре-аудит, R-08): запись ссылается на внешний
    # гео-ассет, если это «geosite:tag», «geoip:tag» (в том числе
    # инвертированные «!…») или «ext:file.dat:tag». Возвращает имя
    # файла ассета либо None.
    ...
    if e.startswith('geosite:'):
        return 'geosite.dat'
    if e.startswith('geoip:'):
        return 'geoip.dat'
    return None


def _needs_geo_assets(rule):
    """Правилу нужны гео-ассеты (geosite/geoip/ext .dat)."""
    domain = rule.get('domain') or []
    if isinstance(domain, str):
        domain = [domain]
    for entry in domain:
        if _geo_asset_ref(entry):
            return True
    ip = rule.get('ip') or []
    if isinstance(ip, str):
        ip = [ip]
...
def _convert_singbox_vless(sb):
    ...
    if security == 'reality':
        ...
        if fp:
            ts["fingerprint"] = fp
        # insecure из входного конфига раньше игнорировался (жёсткое
        # allowInsecure=False), затем ядро Xray ≥ 25.x убрало поле вовсе
        # (true → hard-fail 'xray -test'). dns4.2.18: рабочий путь —
        # pinnedPeerCertSha256 (verifyPeerCertByName сверяет имя ПОВЕРХ
        # CA-проверки и самоподписанный сертификат отклоняет); хеш — из
        # tls.pin_sha256, иначе один TOFU-зонд к серверу. sni здесь всегда
        # непуст (fallback на server).
        if bool(tls.get('insecure', False)):
            sb_pin = (tls.get('pin_sha256')
                      or tls.get('pinnedPeerCertSha256') or '')
            if sb_pin and not _valid_cert_pin(sb_pin):
                raise ValueError(
                    "tls.pin_sha256: ожидается 64 hex-символа SHA-256 по "
                    "DER; base64 ядром Xray не принимается. Получить: %s"
                    % _pin_helper_cmd(server, port, sni))
            sb_pin = (sb_pin or '').lower()
            if not sb_pin:
                sb_pin = _fetch_server_cert_sha256(server, port, sni) or ''
            if not sb_pin:
                raise ValueError(
                    "Xray ≥26 не поддерживает tls.insecure; сертификат "
                    "%s:%s недоступен для автоматического пиннинга. Укажите "
                    "tls.pin_sha256 вручную: %s"
                    % (server, port, _pin_helper_cmd(server, port, sni)))
            ts["pinnedPeerCertSha256"] = sb_pin
            _log_pin_conversion(
                sb_pin, 'vless-reality',
                'sing-box tls.insecure', server, port, sni)
        alpn = tls.get('alpn')
        if alpn:
...
def _convert_singbox_hysteria(sb):
    ...
    if alpn:
        native['tls']['alpn'] = alpn
    # dns4.2.35 (ре-аудит, R-09): лимиты полосы нативный клиент
    # понимает («bandwidth», Мбит/с) — переносим. Порт-хоппинг
    # (список портов сервера и интервал прыжков) нативный клиент
    # не воспроизводит: тихий успех означал бы подключение к
    # единственному порту вместо ожидаемого хоппинга, поэтому
    # такие импорты отклоняем явно.
    if sb.get('server_ports') or sb.get('hop_interval'):
        raise ValueError(
            "hysteria2: port hopping (server_ports/"
            "hop_interval) не поддерживается нативным "
            "клиентом")
    _hy_up = sb.get('up_mbps')
    _hy_down = sb.get('down_mbps')
    if _hy_up or _hy_down:
        _hy_bw = {}
        if _hy_up:
            _hy_bw['up'] = int(_hy_up)
        if _hy_down:
            _hy_bw['down'] = int(_hy_down)
        native['bandwidth'] = _hy_bw
    # dns4.2.32 (статический аудит, F-06): без переноса obfs
    # обфусцированный сервер отвергал бы соединение, хотя импорт
    # выглядел успешным. Неподдерживаемые варианты — явная ошибка,
    # а не тихое игнорирование.
    obfs = sb.get('obfs') or {}
    if obfs:
        obfs_type = obfs.get('type')
        if obfs_type != 'salamander' \
                or not obfs.get('password'):
            raise ValueError(
                "obfs: поддерживается только "
                "salamander с password")
        native['obfs'] = {
            'type': 'salamander',
            'salamander': {
...
def apply_direct_config(protocol,
    ...
    if protocol == 'vless':
        if 'outbounds' in parsed:
            ...
            if not isinstance(rules, list):
                rules = []
            # dns4.2.32 (статический аудит, F-05): прежний фильтр
            # без предупреждения выбрасывал ВСЕ правила с ключом
            # «domain», включая пользовательские исключения
            # (например, домен -> direct). Теперь правила
            # сохраняются; отбрасываются только требующие гео-ассетов
            # (geosite:/geoip:) — проект .dat-файлы не поставляет,
            # и старт xray с ними завершился бы ошибкой.
            cr = []
            _geo_dropped = 0
            for r in rules:
                if not isinstance(r, dict):
                    continue
                if _needs_geo_assets(r):
                    _geo_dropped += 1
                    continue
                cr.append(r)
            if _geo_dropped:
                log_error(
                    "Импорт xray: правил с "
                    "geosite:/geoip: отброшено — "
                    "гео-ассеты отсутствуют: "
                    + str(_geo_dropped))
            if cr:
                rt['rules'] = cr
                parsed['routing'] = rt
    ...
    if protocol == 'vless':
        ...
        parsed = _ensure_xray_tproxy(parsed)
        parsed = _ensure_xray_mux_udp443(parsed)
        parsed = _normalize_xray_insecure(parsed)
    elif protocol == 'hysteria':
        parsed = _ensure_hysteria_tproxy(parsed)
...
def create_backup_with_params(
    ...
    try:
        # Баг 2 фикс: было stderr=PIPE при чтении ТОЛЬКО потока
        # `for line in process.stdout`. Если внешний скрипт бэкапа
        # (KeenSnap/backup_project.sh) выводит на stderr больше объёма
        # буфера канала (~64 КБ), его запись блокируется, а родитель
        # ждёт закрытия stdout — взаимная блокировка, и кнопка «Бэкап»
        # виснет навсегда. Слияние в тот же поток безопасно: не-JSON
        # строки отбрасывает ветка JSONDecodeError ниже, при этом
        # диагностика ошибок остаётся в логе потока.
