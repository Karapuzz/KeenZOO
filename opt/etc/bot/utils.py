    ...
    if not has_catch_all and not has_udp_rule:
        ...
        def _routing_catch_all(r):
            ...
            if isinstance(ip, list) and ip:
                ...
                ip_any = ip.strip() in _any_ip
            for key in ('domain', 'domains', 'source', 'port',
                        'sourcePort', 'protocol', 'geosite',
                        'geoip', 'rule_set', 'inboundTag',
                        'attrs', 'user'):
                if r.get(key):
                    return False
            if ip and not ip_any:
                return False
            return True

        pos = len(rules)
        for i, r in enumerate(rules):
            if _routing_catch_all(r):
                pos = i
                break
