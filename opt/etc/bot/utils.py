...
class DNSPolicyV4:
    ...
    def forward(self, query, tcp=False):
        ...
        # dns4.2.33 (статический аудит, F-01): специальные зоны
        # резолвятся через контроллер (маркированный TCP-сокет через
        # туннель); прямой немаркированный DNS на :53 закрыт фильтром.
...
def _ensure_xray_udp_routing(parsed):
    ...
    has_udp_rule = any(
        _covers_all_udp(r, proxy_tag) for r in rules)
...
def _needs_geo_assets(rule):
    """Правилу нужны гео-ассеты (geosite/geoip .dat)."""
    domain = rule.get('domain') or []
    if isinstance(domain, str):
        domain = [domain]
    for entry in domain:
        if isinstance(entry, str) \
                and entry.strip().startswith('geosite:'):
            return True
    ip = rule.get('ip') or []
    if isinstance(ip, str):
        ip = [ip]
    for entry in ip:
        if isinstance(entry, str) \
                and entry.strip().startswith('geoip:'):
            return True
