...
def parse_vless_key(key, bot=None,
    ...
    return {
        ...
        'spx': params.get(
            'spx', ['/'])[0],
        # dns4.2.17: allowInsecure=1 и алиас insecure=1. Намерение «самопод-
        # писанный сервер» пробрасывается в render (generate_config →
        # _normalize_xray_insecure → verifyPeerCertByName), т.к. на Xray
...
def generate_config(key, template_file,
    ...
    if template_file.endswith('.json'):
        ...
        if template_file == 'vless_template.json':
            ...
            # dns4.2.17: allowInsecure=1 / insecure=1 в ссылке — намерение
            # «самоподписанный сервер»; шаблон несёт allowInsecure:false,
            # поэтому без intent-проброса параметр молча испарялся.
            parsed = _normalize_xray_insecure(
                parsed,
                intent_flag=(str(params.get('allowInsecure', '')).lower()
                             in ('1', 'true', 'yes')),
                intent_name=(params.get('sni')
...
def _normalize_xray_insecure(parsed, intent_flag=None, intent_name=None):
    """allowInsecure → verifyPeerCertByName (самоподписанный сервер).

    Xray ≥ 25.x убрал поле allowInsecure: значение true вызывает hard-fail
    проверки конфига («migrated to pinnedPeerCertSha256 /
    verifyPeerCertByName»), и самоподписанный VPS стало невозможно
    применить. Ближайшая честная замена — verifyPeerCertByName: имя в
    сертификате сверяется, CA-цепочка не требуется, шифрование сохраняется.
    Понижение семантики (нет проверки → сверкa имени) фиксируется
    предупреждением в error.log.

    truthy allowInsecure в outbounds[].streamSettings.tlsSettings
    конвертируется всегда; intent_flag=True дополнительно включает
    конвертацию для всех TLS-outbound'ов (намерение пользователя из ссылки,
    где шаблон ещё нёс allowInsecure:false). intent_name — SNI на такой
    случай, если в tlsSettings нет serverName. Без имени вообще — ValueError.
    """
    if not isinstance(parsed, dict):
        return parsed
    outbounds = parsed.get('outbounds')
    ...
    for ob in outbounds:
        ...
        if not isinstance(tls, dict):
            continue
        tls_seen += 1
        # Явно заданный пользователем verifyPeerCertByName имеет приоритет
        # над конверсией: поле не перезатирается, снимается только
        # недействительный allowInsecure.
        existing = tls.get('verifyPeerCertByName')
        want = bool(tls.get('allowInsecure')) or bool(
            intent_flag and not existing)
        if not want:
            continue
        name = existing or tls.get('serverName') or intent_name
        if not name:
            raise ValueError(
                "Xray ≥26 не поддерживает allowInsecure: без serverName/SNI "
                "конвертировать нельзя — укажите server_name (sni) или задайте "
                "pin вручную (pinnedPeerCertSha256)")
        tls.pop('allowInsecure', None)
        tls['verifyPeerCertByName'] = name
        converted += 1
        log_error(
            "Xray: allowInsecure → verifyPeerCertByName '%s' (outbound '%s'): "
            "ядро ≥26 не принимает allowInsecure; CA-цепочка не проверяется, "
            "сверяется имя в сертификате"
            % (name, ob.get('tag') or ob.get('protocol') or '?'))
    if intent_flag and converted == 0 and tls_seen == 0:
        log_error(
            "Xray: запрошен allowInsecure/insecure=1, но TLS-outbound "
            "не найден (security=reality/none): параметр проигнорирован")
    return parsed
...
def _convert_singbox_vless(sb):
    ...
    if security == 'reality':
        ...
        # сертификатом переставал работать. Затем ядро Xray ≥ 25.x убрало
        # allowInsecure вовсе (true → hard-fail 'xray -test'): теперь —
        # честная замена verifyPeerCertByName (имя сверяется, CA-цепочка
        # не требуется). sni здесь всегда непуст (fallback на server).
        if bool(tls.get('insecure', False)):
            ts["verifyPeerCertByName"] = sni
            log_error(
                "Xray: sing-box tls.insecure → verifyPeerCertByName "
                "'%s': ядро ≥26 не принимает allowInsecure; CA-цепочка "
                "не проверяется, сверяется имя в сертификате" % sni)
        alpn = tls.get('alpn')
        if alpn:
