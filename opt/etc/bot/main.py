    ...
    while restart_count < config.MAX_RESTARTS:
        try:
            ...
            if transient:
                ...
                continue
            restart_count += 1
            # dns4.2.27 (внешний аудит, H-03): экспоненциальный бэкофф
            # с потолком 300 с — флэп рестартов больше не долбит
            # Telegram API сериями быстрых переподключений (429).
            time.sleep(min(config.RESTART_DELAY * (2 ** (restart_count - 1)),
                           300))
        except SystemExit:
            raise
