...
from menu import (
    ...
    BackupStates, create_drive_selection_menu,
...
def setup_handlers(bot):
    ...
    def handle_add_to_bypass(message):
        ...
        for s in existing_sections:
            for entry in s['entries']:
                ...
                if clean:
                    ck = clean.lower()
                    if ck not in existing_clean_map:
                        existing_clean_map[ck] = (
                            entry, s['comment'])
        is_bot = (
                ...
                if key in input_seen:
                    ...
                    duplicate_msgs.append(
                        ...
                        + (f" (#{fc})"
                           if fc else ""))
                    continue
                input_seen[key] = comment
                if key in existing_clean_map:
                    _, ec = (
                        existing_clean_map[key])
                    duplicate_msgs.append(
                        f"• {clean} → уже"
                        + (f" в #{ec}" if ec
                           else " в списке"))
                    continue
                if key in global_clean_map:
                    info = global_clean_map[key]
                    duplicate_msgs.append(
                        f"• {clean} → в "
                        f"{info['file']}.txt"
                        + (f" (#{info['comment']})"
        ...
        if added_count > 0:
            ...
            bot.send_message(
                ...
                '\n'.join(msg_parts))
            try:
                # Как и в веб-панели (_save_list_route), применяем
                # только затронутый набор: полный цикл резолвит все
                # списки подряд при MAX_PARALLEL=4 десятки секунд.
                _load_gen().apply_unblock_async(
                    config.ipset_names.get(selected, ''))
            except (RuntimeError, OSError) as e:
                bot.send_message(
                    message.chat.id,
                    f"❌ Применение не запущено: {e}\n"
                    f"Правка сохранена и применится "
                    f"при следующем обновлении.")
        if duplicate_msgs:
            bot.send_message(
                message.chat.id,
                "❕Дубликаты:\n"
    ...
    def handle_restart(chat_id):
        ...
        # dns4.2.30 (полный аудит, П1): атомарная запись — окно между
        # усечением и записью при потере питания оставляло пустой файл,
        # который ломал каждый следующий запуск бота (см.
        # utils.check_restart).
        _restart_tmp = config.paths["chat_id_path"] + ".tmp"
        with open(_restart_tmp, 'w') as f:
            f.write(str(chat_id))
            f.flush()
            os.fsync(f.fileno())
        os.replace(_restart_tmp, config.paths["chat_id_path"])
        # dns4.2.27 (внешний аудит, H-02): отдельная сессия — сигнал
        # на рестарт бота не задевает дочерний сервисный скрипт
        # (паритет со всеми Popen генератора).
        subprocess.Popen(
    ...
    def handle_updates(chat_id):
        ...
        # dns4.2.20 (аудит A2 §3): ветка сравнения версий бота удалена как
        # мёртвый код — get_remote_version() возвращает get_local_version()
        # (внешний источник удалён), равенство истинно всегда, поэтому
        # need_bot_update не наступал никогда и кнопка «Обновить бота»
        # была недостижима.
    ...
    def handle_backup_return(call):
        if not is_allowed_call(call):
            return
        backup_state = backup_states.get(
