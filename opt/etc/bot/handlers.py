    ...
    def bot_message(message):
        ...
        if message.text in MENU_TRANSITIONS:
            MENU_TRANSITIONS[message.text](
                message.chat.id)
        elif (state.current_menu.level
              in LEVEL_HANDLERS):
            LEVEL_HANDLERS[
                state.current_menu.level](
                    message)
        else:
            bot.send_message(
                message.chat.id,
