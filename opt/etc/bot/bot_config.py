...
# dns4.2.32 (статический аудит, F-08): подсети, которым доступна
# веб-панель. По умолчанию — петля и RFC1918 (LAN). Если панель
# нужна из VPN-сети с адресами вне RFC1918 (например, пиры
# WireGuard в CGNAT 100.64.0.0/10), добавьте её сюда, напр.
# '100.64.0.0/10'.
...
# (dns4.2.23, аудит A5 #5: deprecated-дубль localportrouter удалён —
# потребителей не было с dns4.2.20; реальный переключатель —
...
web_port = 8080
# dns4.2.23 (аудит A5 #2): имена, по которым пользователи открывают панель.
# Используются в Origin/Referer-проверке вместо заголовка Host самого
# запроса: он контролируется атакующим при DNS-rebinding и потому не
# может служить источником доверия.
router_hostnames = ['my.keenetic.net']
generator_settings = {
    'listen_ip': routerip,
