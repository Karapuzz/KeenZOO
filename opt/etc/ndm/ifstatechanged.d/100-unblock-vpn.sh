...
(
    ...
    # dns4.2.37 H-2 / dns4.2.38 N-6: статус читается ДО выбора ветки.
    # При отложенной транзакции (статус «defer») или пустом статусе
    # (свежая загрузка, /tmp очищен) выполняется только
    # unblock_update.sh: он САМ вызывает unblock_dnsmasq.sh (стадия
    # кандидата и финальная публикация), поэтому прежний прямой вызов
    # перед ним давал двойной полный прогон на каждое поднятие WAN —
    # двойная работа и лишний рестарт dnsmasq в самый чувствительный
    # момент. Параллельный запуск невозможен: у скрипта обновления
    # собственный каталог-лок. В остальных состояниях — только прямой
    # вызов обновления пинов/интерфейсов, как прежде.
    _wd_defer_st="$(grep -o '"status"[[:space:]]*:[[:space:]]*"[^"]*"' \
        /tmp/unblock_update_status.json 2>/dev/null | head -1)"
    _wd_run_update=0
    case "$_wd_defer_st" in
        *defer* | '') [ -x /opt/bin/unblock_update.sh ] && _wd_run_update=1 ;;
    esac
    if [ "$_wd_run_update" = 1 ]; then
        logger -t "$TAG" "WAN up: повтор отложенного/первичного обновления списков"
        if /opt/bin/unblock_update.sh >/dev/null 2>&1; then
            logger -t "$TAG" "WAN up: списки, пины и интерфейсы dnsmasq обновлены"
        else
            _wd_dns_rc=$?
            logger -t "$TAG" "WAN up: обновление списков отложено/не удалось rc=$_wd_dns_rc"
        fi
    elif [ -x /opt/bin/unblock_dnsmasq.sh ]; then
