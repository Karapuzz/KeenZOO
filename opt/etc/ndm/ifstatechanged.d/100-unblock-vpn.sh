...
(
    ...
    # dns4.2.37 H-2: отложенная транзакция обновления (статус «defer»,
    # rc 75 из-за недоступного DNS) сама не повторяется до следующего
    # окна по расписанию (06:00). После восстановления WAN/туннеля
    # среда обычно снова пригодна — даём отложенной транзакции один
    # внеплановый повтор здесь. Пустой статус (свежая загрузка, /tmp
    # очищен) тоже покрывается: пинам нужен первичный проход.
    # Параллельный запуск невозможен: unblock_update.sh сам владеет
    # своим каталогом-локом и не стартует поверх активной транзакции.
