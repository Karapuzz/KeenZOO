...
if [ "${type:-}" = "ip6tables" ]; then
    ...
    # dns4.2.35 (ре-аудит, R-11): проектные правила удаляются ВСЕГДА
    # (старые варианты не должны опережать новые), а при удалении
    # проекта на этом работа заканчивается — ДО установки новых
    # правил. Раньше проверка шла после установки, и -remove
    # оставлял свежие IPv6 DROP-правила на уже несуществующие сервисы.
    ip6_drop_project_rules() {
...
purge_project_netfilter() {
    ...
    # dns4.2.35 (ре-аудит, R-03): без снятия цепочки закрытия
    # немаркированного DNS после удаления системный резолвер
    # прошивки получал бы REJECT на собственных восходящих запросах.
    for _pps_chain in KZ_DNS_V4_FILTER KZ_DNS_V4_FILTER_NEW; do
        for _pps_p in tcp udp; do
            while $IPT -D OUTPUT -p "$_pps_p" --dport 53 \
                -j "$_pps_chain" >/dev/null 2>&1; do :; done
        done
        $IPT -F "$_pps_chain" >/dev/null 2>&1 || true
        $IPT -X "$_pps_chain" >/dev/null 2>&1 || true
    done
    ...
    for _pps_if in $_pps_all_ifaces; do
        # dns4.2.37 M-2: установка перехватывает клиентский :53 через
        # `REDIRECT --to-ports 53`, а при недоступном xt_REDIRECT —
        # фолбэком `DNAT --to-destination <IP самого интерфейса>` (он
        # может отличаться от $local_ip). Раньше чистился только DNAT на
        # $local_ip, и после -remove на каждом интерфейсе оставались до
        # двух правил перехвата клиентского DNS.
        _pps_if_ip="$(ip -4 addr show "$_pps_if" 2>/dev/null \
        ...
        for _pps_proto in tcp udp; do
            ...
            while $IPT -t nat -D PREROUTING -i "$_pps_if" -p "$_pps_proto" \
                --dport 53 -j REDIRECT --to-ports 53 >/dev/null 2>&1; do :; done
            if [ -n "$_pps_if_ip" ] && [ "$_pps_if_ip" != "$local_ip" ]; then
                while $IPT -t nat -D PREROUTING -i "$_pps_if" -p "$_pps_proto" \
                    --dport 53 -j DNAT --to-destination "$_pps_if_ip" \
    ...
    for _pps_proto in tcp udp; do
        ...
        # dns4.2.37 M-2: и безынтерфейсная форма клиентского перехвата.
        while $IPT -t nat -D PREROUTING -p "$_pps_proto" --dport 53 \
            -j REDIRECT --to-ports 53 >/dev/null 2>&1; do :; done
    done
    for _pps_spec in \
...
if [ "${KEENZOO_UPDATE_LOCK_HELD:-0}" != "1" ]; then
    ...
    while ! mkdir "$NETFILTER_LOCK_DIR" 2>/dev/null; do
        ...
        # dns4.2.37 M-1: 30 попыток × сон держали хук до 30+ с на
        # каждое событие при занятом локе — на MIPS это риск таймаута
        # NDM. Пяти секунд ожидания достаточно: дальше срабатывает
        # существующий однократный фоновый повтор (маркер ниже),
        # который и доводит правила до конца без блокировки хука.
        if [ "$_nfl_tries" -ge 5 ]; then
...
pick_iptables() {
    ...
    # dns4.2.27 (внешний аудит, R-02): кандидаты из /tmp исключены —
    # /tmp world-writable, бинарь оттуда можно подменить; паритет с
    # фиксом A3 C1 (dns4.2.21), который закрыл их в deploy, но пропустил
    # этот хук.
...
# dns4.2.28 (внешний отчёт, P-07): блок PORT_* здесь дублировал сид выше
# (оба вида ${VAR:-…} — повтор был функционально пуст). Оставлены только
# константы, которых в сиде нет.
...
svc_enabled() {
    ...
    # dns4.2.37 H-3 / dns4.2.38 N-2: фастпуть через pidof — ОДИН exec
    # вместо обхода всего /proc с тремя форками на каждый PID (~100 PID
    # на MIPS). pidof сравнивает имя процесса, а не подстроку командной
    # строки, поэтому «vi /opt/etc/hysteria/config.json» живым сервисом
    # не считается — той же строгости, что у обхода ниже. Отрицательный
    # ответ столь же надёжен: оба способа сравнивают одно и то же, и
    # проваливание в обход стоило по ~3 форка на каждый PID системы на
    # КАЖДЫЙ выключенный сервис (2–3 из 5 в типовой установке). Обход
    # /proc остаётся только для сборок, где нет самого pidof.
    if command -v pidof >/dev/null 2>&1; then
        if pidof "$_svc_proc" >/dev/null 2>&1; then
...
# dns4.2.35 (ре-аудит, R-02/R-03): содержимое цепочки статично, поэтому
# обычный прогон — быстрая сверка готовности (без единого изменения в
# рабочем пути ядра). Если цепочка отсутствует или повреждена, она
# пересобирается через стейджинг-цепочку: новая заполняется ПОЛНОСТЬЮ,
# прыжок переключается на неё и лишь затем снимается старая — окна
# «пустая цепочка возвращает пакет в OUTPUT» нет. Оба имени равноправны;
# активна та цепочка, на которую сейчас указывает прыжок.
KZ_RAW_GUARD_CHAINS="KZ_DNS_V4_FILTER KZ_DNS_V4_FILTER_NEW"

kzv4_raw_guard_chain_ok() {
    _rgco_chain="$1"
    $IPT -C "$_rgco_chain" -d 127.0.0.0/8 -p tcp --dport 53 \
        -j RETURN >/dev/null 2>&1 || return 1
    $IPT -C "$_rgco_chain" -d 127.0.0.0/8 -p udp --dport 53 \
        -j RETURN >/dev/null 2>&1 || return 1
    $IPT -C "$_rgco_chain" -p tcp --dport 53 -m mark \
        --mark 0x2000104 -j RETURN >/dev/null 2>&1 || return 1
    $IPT -C "$_rgco_chain" -p udp --dport 53 -m mark \
        --mark 0x2000104 -j RETURN >/dev/null 2>&1 || return 1
    $IPT -C "$_rgco_chain" -p tcp --dport 53 \
        -j REJECT >/dev/null 2>&1 || return 1
    $IPT -C "$_rgco_chain" -p udp --dport 53 \
        -j REJECT >/dev/null 2>&1 || return 1
    return 0
}

kzv4_raw_guard_wanted() {
    for _rgw_chain in $KZ_RAW_GUARD_CHAINS; do
        kzv4_raw_guard_chain_ok "$_rgw_chain" || continue
        for _rgw_p in tcp udp; do
            $IPT -C OUTPUT -p "$_rgw_p" --dport 53 \
                -j "$_rgw_chain" >/dev/null 2>&1 || return 1
        done
        return 0
    done
    return 1
...
kzv4_raw_guard_remove() {
    for _rgr_chain in $KZ_RAW_GUARD_CHAINS; do
        for _rgr_p in tcp udp; do
            while $IPT -D OUTPUT -p "$_rgr_p" --dport 53 \
                -j "$_rgr_chain" >/dev/null 2>&1; do :; done
        done
        $IPT -F "$_rgr_chain" >/dev/null 2>&1 || true
        $IPT -X "$_rgr_chain" >/dev/null 2>&1 || true
    done
}

apply_dns_v4_raw_guard() {
    # dns4.2.33 (статический аудит, F-01): в режиме DNSv4 немаркированный
    # внешний TCP/UDP 53 закрывается. Разрешены ТОЛЬКО петля (dnsmasq и
    # контроллер; спец-зоны тоже идут через контроллер) и аварийная метка
    # 0x2000104 (у контроллера все апстрим-сокеты маркированы, поэтому
    # других исключений не нужно). Маркированные кандидаты туннеля, как и
    # прежде, уходят через NAT, а при сбое NAT их отклоняют гарды выше.
    # Во время установки/удаления цепочка снимается: установщик сам может
    # временно ходить на публичные резолверы (окно подмены resolv.conf).
    if [ -f /tmp/keenzoo_installing ] \
        || [ -f /opt/etc/unblock/.disabled ]; then
        kzv4_raw_guard_remove
        return 0
    fi
    kzv4_raw_guard_wanted && return 0
    # dns4.2.37 M-4: стейджинг ВСЕГДА брал имя _NEW. Если активной была
    # именно _NEW-цепочка (нормальное состояние после первого цикла),
    # пересборка делала `-F` по ЖИВОЙ цепочке (окно без защиты) и
    # вставляла второй джамп поверх первого. Теперь стейджинг — то из
    # двух имён, на которое сейчас НЕТ джампа в OUTPUT: чистится и
    # наполняется только неактивная цепочка, переключение атомарно на
    # уровне джампов. Если по какой-то причине джампы есть на обе —
    # стейджингом берётся базовое имя, а цикл снятия «прежних» ниже
    # удаляет ВСЕ джампы на _NEW, разбирая дубли.
    _rg_output_dump="$($IPT -S OUTPUT 2>/dev/null || true)"
    _rg_staging="KZ_DNS_V4_FILTER_NEW"
    case "$_rg_output_dump" in
        *"-j KZ_DNS_V4_FILTER_NEW"*) _rg_staging="KZ_DNS_V4_FILTER" ;;
    esac
    $IPT -N "$_rg_staging" >/dev/null 2>&1 || true
    $IPT -F "$_rg_staging" || return 1
    for _rg_proto in tcp udp; do
        $IPT -A "$_rg_staging" -d 127.0.0.0/8 \
            -p "$_rg_proto" --dport 53 -j RETURN || return 1
        $IPT -A "$_rg_staging" -p "$_rg_proto" --dport 53 \
            -m mark --mark 0x2000104 -j RETURN || return 1
    done
    $IPT -A "$_rg_staging" -p tcp --dport 53 -j REJECT || return 1
    $IPT -A "$_rg_staging" -p udp --dport 53 -j REJECT || return 1
    for _rg_proto in tcp udp; do
        $IPT -I OUTPUT 1 -p "$_rg_proto" --dport 53 \
            -j "$_rg_staging" || {
                # Откат стейджинга без дыры: старые правила не тронуты.
                for _rg_rb in tcp udp; do
                    while $IPT -D OUTPUT -p "$_rg_rb" --dport 53 \
                        -j "$_rg_staging" >/dev/null 2>&1; do :; done
                done
                $IPT -F "$_rg_staging" >/dev/null 2>&1 || true
                $IPT -X "$_rg_staging" >/dev/null 2>&1 || true
                return 1
            }
    done
    # Джамп уже указывает на готовую цепочку — снимаем прежнюю.
    for _rg_chain in $KZ_RAW_GUARD_CHAINS; do
        [ "$_rg_chain" = "$_rg_staging" ] && continue
        for _rg_proto in tcp udp; do
            while $IPT -D OUTPUT -p "$_rg_proto" --dport 53 \
                -j "$_rg_chain" >/dev/null 2>&1; do :; done
        done
        $IPT -F "$_rg_chain" >/dev/null 2>&1 || true
        $IPT -X "$_rg_chain" >/dev/null 2>&1 || true
    done
    return 0
}
...
apply_dns_filter() {
    ...
    if [ "${DNS_POLICY_V4:-0}" = 1 ]; then apply_dns_v4_raw_guard || return 1; fi
...
# dns4.2.38 N-4а: вычистка только при наличии правил набора в дампе —
# под DNSv4 переменная DNS_TUNNEL_PORT всегда пуста, и 9-12 пробных -D
# на каждом проходе были чистой миграцией, которой давно нечего чистить.
_dn_out_dump="$($IPT -t nat -S OUTPUT 2>/dev/null || true)"
case "$_dn_out_dump" in
    *"--match-set unblockdns dst"*)
...
if [ "${table:-}" = "filter" ]; then
    ...
    # dns4.2.38 N-3: миграционная вычистка устаревших «-s <сеть>»
    # разрешений (66 пробных -D на каждое событие) запускается только
    # при наличии сигнатуры в дампе. Ложное срабатывание гейта (чужое
    # правило с той же сетью) лишь возвращает прежнее поведение.
    _kz_in_dump="$($IPT -S INPUT 2>/dev/null || true)"
    _kz_legacy_src=0
    for _kz_net in $LOCAL_NETS; do
        case "$_kz_in_dump" in
            *"-s $_kz_net "*) _kz_legacy_src=1; break ;;
        esac
    done
...
#
# dns4.2.37 H-3: миграционные циклы «на всякий случай» запускаются
# только при наличии их сигнатуры в дампе таблицы. Один `-S`-дамп
# (переменная ниже используется и в секции 4) вместо десятков пробных
# `-D` на каждом событии, когда вычищать уже нечего.
_kz_pre_dump="$($IPT -t nat -S PREROUTING 2>/dev/null || true)"
_kz_legacy_dnat=0
case "$_kz_pre_dump" in
    *"--to-destination $local_ip"*) _kz_legacy_dnat=1 ;;
esac
if [ "$_kz_legacy_dnat" = 1 ]; then
...
for iface in $LAN_IFACES; do
    for protocol in udp tcp; do
        ...
        # (dns4.2.37 H-3: гейт по сигнатуре в дампе).
        if [ "$_kz_legacy_dnat" = 1 ]; then
...
#
# dns4.2.37 H-3: всё, что чистит эта секция, — правила БЕЗ -i. Актуальные
# правила все с -i, поэтому при их отсутствии в дампе чистить нечего и
# десять пробных `-D`-циклов (по два-три exec каждый) пропускаются. Один
# разбор уже снятого дампа вместо ~30 вызовов iptables на событие.
_kz_legacy_no_if=0
case "$_kz_pre_dump" in
    *"--match-set unblock"*)
        if printf '%s\n' "$_kz_pre_dump" \
            | grep -- '--match-set unblock' | grep -vq -- '-i '; then
            _kz_legacy_no_if=1
        fi
        ;;
esac
if [ "$_kz_legacy_no_if" = 1 ]; then
...
for iface in $LAN_IFACES; do
    if svc_enabled "$INIT_SS"; then
        ...
        # dns4.2.37 M-3: UDP для ss-redir идёт через TPROXY в секции 5.
        # После nat/REDIRECT адресом назначения в заголовке становится
        # адрес роутера, а UDP-релей ss-redir получает исходный адрес
        # через IP_RECVORIGDSTADDR, что работает только с TPROXY —
        # старый UDP REDIRECT отправлял трафик «в никуда». Здесь только
        # миграционное снятие устаревшего правила.
        # dns4.2.38 N-4б: миграция запускается только при наличии
        # сигнатуры в дампе, снятом в начале секции 1.
        case "$_kz_pre_dump" in
            *"-p udp -m set --match-set unblocksh dst"*)
...
# ═════════════════════════════════════════════════════════════════════════
# 5. UDP через TPROXY для VLESS (xray), Hysteria2 и Shadowsocks
# ═════════════════════════════════════════════════════════════════════════
...
# Для shadowsocks то же ограничение: ss-redir узнаёт исходный адрес
# назначения только из сокета с IP_RECVORIGDSTADDR, т.е. из-под TPROXY
# (dns4.2.37, M-3).
TPROXY_REQUIRED=0
if svc_enabled "$INIT_VLESS" || svc_enabled "$INIT_HY" || svc_enabled "$INIT_SS"; then
    TPROXY_REQUIRED=1
fi

# dns4.2.35 (ре-аудит, R-06): файл состояния TPROXY — построчные итоги
# применения на КАЖДОМ интерфейсе (вместо «есть хотя бы одно правило»
# в деплое): «ok|fail|degraded <iface> <ipset> <port> <причина>».
# Хук — единственный, кто знает правду о применённых правилах.
TPROXY_STATE="/opt/etc/unblock/.tproxy_state"
: > "$TPROXY_STATE" 2>/dev/null || true
tproxy_state_note() {
    printf '%s %s %s %s %s\n' "$1" "$2" "$3" "$4" "$5" \
        >> "$TPROXY_STATE" 2>/dev/null || true
}
...
if [ "$TPROXY_REQUIRED" -eq 1 ]; then
    ...
    # dns4.2.36 (аудит, KZ-13): фиксированные ресурсы маршрутизации
    # (приоритет/таблица) проверяются на чужого владельца ЯВНО, а не
    # используются молча совместно. Конфликт не ломает установку —
    # фиксируется в файле состояния (виден в статусе деплоя) и в логе.
    _kz_tpm_dec="$(printf '%d' "$TPROXY_MARK" 2>/dev/null || true)"
    if ip rule show 2>/dev/null \
        | awk -v p="$TPROXY_RULE_PRIO:" '$1 == p' \
        | grep -Ev "fwmark ($TPROXY_MARK|${_kz_tpm_dec:-0})" | grep -q .; then
        tproxy_state_note degraded - - - rule-$TPROXY_RULE_PRIO-foreign
        log_msg "TPROXY: приоритет $TPROXY_RULE_PRIO занят чужим правилом — ресурсы делятся с другой конфигурацией"
    fi
    if ip route show table "$TPROXY_TABLE" 2>/dev/null \
        | grep -v '^local default dev lo' | grep -q .; then
        tproxy_state_note degraded - - - table-$TPROXY_TABLE-foreign
        log_msg "TPROXY: таблица $TPROXY_TABLE содержит чужие маршруты — ресурсы делятся с другой конфигурацией"
    fi
...
#
# Полировка 4.2.26: добавление гейтится флагом. При деградации ядра
# (префлайт обнулил TPROXY_REQUIRED) не делаем по два холостых форка
# iptables и по две строки журнала на каждый интерфейс на каждое сетевое
# событие. ВНИМАНИЕ: ветки вычистки (mangle_del_tproxy) гейтить нельзя —
# ноль в флаге бывает и при ВЫКЛЮЧЕННЫХ сервисах на живом ядре, и тогда
# именно эти вызовы снимают правила, оставшиеся от включённого состояния.
for iface in $LAN_IFACES; do
    if svc_enabled "$INIT_SS"; then
        if [ "$TPROXY_REQUIRED" -eq 1 ]; then
            if mangle_add_tproxy unblocksh "$PORT_SS" "$iface"; then
                tproxy_state_note ok "$iface" unblocksh "$PORT_SS" -
            else
                log_msg "TPROXY: правило для $iface отклонено — деградация (интерфейс/протокол пропущен)"
                tproxy_state_note fail "$iface" unblocksh "$PORT_SS" rule-rejected
            fi
        fi
    else
        mangle_del_tproxy unblocksh       "$PORT_SS"      "$iface"
    fi
    if svc_enabled "$INIT_VLESS"; then
        if [ "$TPROXY_REQUIRED" -eq 1 ]; then
            if mangle_add_tproxy unblockvless "$PORT_VLESS" "$iface"; then
                tproxy_state_note ok "$iface" unblockvless "$PORT_VLESS" -
            else
                log_msg "TPROXY: правило для $iface отклонено — деградация (интерфейс/протокол пропущен)"
                tproxy_state_note fail "$iface" unblockvless "$PORT_VLESS" rule-rejected
            fi
        fi
    else
    ...
    if svc_enabled "$INIT_HY"; then
        if [ "$TPROXY_REQUIRED" -eq 1 ]; then
            if mangle_add_tproxy unblockhysteria "$PORT_HYSTERIA" "$iface"; then
                tproxy_state_note ok "$iface" unblockhysteria "$PORT_HYSTERIA" -
            else
                log_msg "TPROXY: правило для $iface отклонено — деградация (интерфейс/протокол пропущен)"
                tproxy_state_note fail "$iface" unblockhysteria "$PORT_HYSTERIA" rule-rejected
            fi
        fi
    else
