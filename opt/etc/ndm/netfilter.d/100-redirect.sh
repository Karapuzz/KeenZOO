...
kzv4_raw_guard_remove() {
    for _rg_proto in tcp udp; do
        while $IPT -D OUTPUT -p "$_rg_proto" --dport 53 \
            -j KZ_DNS_V4_FILTER >/dev/null 2>&1; do :; done
    done
}
...
apply_dns_v4_raw_guard() {
    ...
    $IPT -N KZ_DNS_V4_FILTER >/dev/null 2>&1 || true
    $IPT -F KZ_DNS_V4_FILTER || return 1
    for _rg_proto in tcp udp; do
        $IPT -A KZ_DNS_V4_FILTER -d 127.0.0.0/8 \
            -p "$_rg_proto" --dport 53 -j RETURN || return 1
        $IPT -A KZ_DNS_V4_FILTER -p "$_rg_proto" --dport 53 \
            -m mark --mark 0x2000104 -j RETURN || return 1
    done
    $IPT -A KZ_DNS_V4_FILTER -p tcp --dport 53 -j REJECT || return 1
    $IPT -A KZ_DNS_V4_FILTER -p udp --dport 53 -j REJECT || return 1
    for _rg_proto in tcp udp; do
        while $IPT -D OUTPUT -p "$_rg_proto" --dport 53 \
            -j KZ_DNS_V4_FILTER >/dev/null 2>&1; do :; done
        $IPT -I OUTPUT 1 -p "$_rg_proto" --dport 53 \
            -j KZ_DNS_V4_FILTER || return 1
    done
}
