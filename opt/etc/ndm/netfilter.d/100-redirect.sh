...
svc_enabled() {
    ...
    # dns4.2.37 H-3: фастпуть через pidof — ОДИН exec вместо обхода
    # всего /proc с тремя форками на каждый PID (~100 PID на MIPS).
    # pidof сравнивает имя процесса, а не подстроку командной строки,
    # поэтому «vi /opt/etc/hysteria/config.json» живым сервисом не
    # считается — той же строгости, что у обхода ниже. Если в сборке
    # нет pidof или он ничего не вернул, управление передаётся
    # прежнему обходу /proc (он остаётся фолбэком, а не удаляется).
